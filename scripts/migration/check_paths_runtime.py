...
Quatre vérifications, via le CODE RÉEL (pas de réimplémentation des chemins) :
  ...
  1. PROMPTS_DIR : `_STATIC_PROMPT_PATH` de crush.engine.agent (le vrai
     constant utilisé par le serveur) est lisible et non-vide.
  2. MEMORY_DATA_DIR : `MemoryKernel` ouvre/crée crush_memory.db, log un
     event, le relit (write/read round-trip via le code réel).
  3. UI_STATIC_DIR : asset statique servi via `TestClient` sur l'app
     `crush.app.app` (mount("/static", ...) et endpoint /admin).
...
def check_1_prompts_dir() -> tuple[bool, str]:
    ...
    try:
        from crush.engine.agent import _STATIC_PROMPT_PATH  # lazy: diagnostic
...
def check_2_memory_data_dir() -> tuple[bool, str]:
    ...
    """
    try:
        from crush.providers.memory.kernel import MemoryKernel  # lazy: diagnostic
    except Exception as e:
        return False, f"import MemoryKernel a levé : {e!r}"

    with tempfile.TemporaryDirectory(prefix="crush_b7b_") as tmp:
...
def check_3_ui_static_dir() -> tuple[bool, str]:
    ...
    On instancie l'app via `from crush.app import app` (le vrai composition
    ...
    try:
        ...
        from crush.app import app  # lazy: diagnostic
    except Exception as e:
    ...
    if "Crush" not in r.text and "views" not in r.text and "register" not in r.text:
...
def check_4_skills_installed_dir() -> tuple[bool, str]:
    ...
    try:
        from crush.capabilities.skills.registry import SkillRegistry  # lazy: diagnostic
    except Exception as e:
