...
dans ~/.crush/extensions/dev/presets/<name>, puis dans skills/installed/
<name>. La sécurité réelle reste assurée par :
  - le validateur statique côté Crush-skills (champ requires_confirmation),
...
def find_preset(name: str) -> Path | None:
    ...
    candidates = [
        ...
        dev_root() / "skills" / name,  # certains presets vivent côté skills/ (cf. Crush-skills)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if preset_dir is None:
        sys.stderr.write(
            ...
            f"(~/.crush/extensions/dev/) ni dans skills/installed/.\n"
        )
        return 1
