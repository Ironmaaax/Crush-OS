...
async def main() -> int:
    ...
    print(f"  audit log : {project.workspace_path}/.crush/audit.jsonl")
    ...
    for f in sorted(ws.rglob("*")):
        if f.is_file() and ".crush" not in str(f):
            print(f"  {f.relative_to(ws)} ({f.stat().st_size} B)")

    # Audit log
    audit_path = ws / ".crush" / "audit.jsonl"
