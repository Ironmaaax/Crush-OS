...
"""Lie une extension `crush-skills` dans la zone dev de Crush (symlink).

Usage :
    python scripts/install_local_extension.py ../crush-skills/skills/mon-skill
    python scripts/install_local_extension.py ../crush-skills/presets/mon-preset
    python scripts/install_local_extension.py ../crush-skills/views/ma-vue
...
Comportement (cf. CDC §1) :
  ...
  2. Valide d'abord avec `<repo>/scripts/validate_catalog.py` du chantier
     `crush-skills`. Refus si le validateur n'existe pas ou retourne ≠ 0.
  3. Crée un symlink `~/.crush/extensions/dev/<type>/<name>` → source.
...
def detect_type(src: Path) -> str:
    ...
    validate_catalog.py côté crush-skills). Heuristique en fallback.
    """
    ...
    if ext_type == "view":
        ...
        if view_md.exists():
            ...
            if text.startswith("---"):
                ...
                # id prioritaire (utilisé par Crush.views.register), sinon name
...
def run_validate_catalog(src: Path) -> None:
    """Appelle <repo_crush-skills>/scripts/validate_catalog.py sur la source.

    Refuse proprement si le validateur n'existe pas (chantier crush-skills
    ...
    if not validator.exists():
        raise SystemExit(
            ...
            "Ce script suppose le chantier `crush-skills` livré "
            "(scripts/validate_catalog.py). Valide ton extension côté "
            "crush-skills d'abord, ou aligne le repo amont."
        )
    ...
    if proc.returncode != 0:
        ...
        raise SystemExit(
            ...
            "Corrige l'extension côté crush-skills avant de la lier."
        )
...
def dev_root() -> Path:
    ...
    override = os.environ.get("CRUSH_DEV_EXTENSIONS_DIR")
    if override:
        return Path(override)
    return Path.home() / ".crush" / "extensions" / "dev"
...
def print_next_steps(ext_type: str, name: str, src: Path) -> None:
    ...
    if ext_type == "skill":
        print(f"  Skill   : relance Crush et essaie : « utilise {name} pour … »")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Lie une extension crush-skills en zone dev.")
