...
"""Prévisualise une vue Crush sans lancer le core complet (cf. CDC §2).

Usage :
    python scripts/preview_view.py ../crush-skills/views/ma-vue

Sert une page HTML qui charge le vrai _shared.js (donc le vrai contrat
Crush.views.register / activate / dispatch / deactivate), le view.js
cible, et expose une mini-UI pour envoyer les mêmes messages que Crush
...
SHARED_JS = REPO / "src" / "crush" / "interfaces" / "ui" / "static" / "_shared.js"
...
<!-- Contrat Crush.views (vrai _shared.js, source unique de vérité) -->
...
(function () {{
  ...
  function applyServerMessage(data) {{
    if (data.type === 'show_view') {{
      window.Crush.views.activate(data.view_id, data.params || {{}});
      log('activate ' + data.view_id); return;
    }}
    if (data.type === 'hide_view') {{
      window.Crush.views.deactivate(data.view_id);
      log('deactivate ' + data.view_id); return;
    }}
    if (data.type === 'view_command') {{
      window.Crush.views.dispatch(data.view_id, data.command, data.params || {{}});
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Prévisualise une vue Crush sans lancer le core.")
    parser.add_argument(
        "view_dir",
        type=Path,
        help="Dossier source de la vue (../crush-skills/views/<name>).",
    )
