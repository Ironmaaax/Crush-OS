...
function Ensure-Bundle {
    ...
    Write-Host ""
    $staging = Join-Path $env:TEMP "crush-bundle-download"
    Remove-Item $staging -Recurse -Force -ErrorAction SilentlyContinue
    New-Item -ItemType Directory -Path $staging -Force | Out-Null
    $zipPath = Join-Path $staging "crush-offline.zip"
    try {
    ...
    Get-CimInstance Win32_Process -ErrorAction SilentlyContinue | ForEach-Object {
        ...
        if ($_.Name -eq "python.exe") {
            ...
            elseif ($cmd -match $rootPattern -and
                    $cmd -match "(-m\s+crush\b|main\.py|voice_agent\.py|bundle\\\.venv)") { $kill = $true }
        }
...
function Clear-CrushStaleLogDirs {
    Get-ChildItem -Path $env:TEMP -Directory -ErrorAction SilentlyContinue |
        Where-Object { $_.Name -match "^crush(\.stale\.|[-_])" } |
        Where-Object { $_.LastWriteTime -lt (Get-Date).AddHours(-6) } |
        ForEach-Object { Remove-Item $_.FullName -Recurse -Force -ErrorAction SilentlyContinue }
}

function Initialize-CrushLogDir {
    Stop-CrushRuntime
    Clear-CrushStaleLogDirs
    Start-Sleep -Milliseconds 400

    $preferred = Join-Path $env:TEMP "crush"
    for ($attempt = 1; $attempt -le 3; $attempt++) {
        try {
            ...
            try {
                if (Test-Path $preferred) {
                    $stale = Join-Path $env:TEMP ("crush.stale." + [DateTimeOffset]::UtcNow.ToUnixTimeSeconds())
    ...
    $fallback = Join-Path $env:TEMP ("crush-" + $PID)
    New-Item -ItemType Directory -Path $fallback -Force | Out-Null
    return $fallback
}

function Invoke-CrushRun {
    $apiPort = [int](Get-DotEnvValue -Key "PORT" -Default "8000")
    Write-Host ""
    Write-Host "  C R U S H" -ForegroundColor Cyan
    ...
    $python = Get-CrushPython
    if ($python) { & $python -m crush.kernel.preflight } else { uv run python -m crush.kernel.preflight }
    ...
    $python = Get-CrushPython
    $apiCmd = if ($python) { "`"$python`" -m crush.app" } else { "uv run python -m crush.app" }
    ...
    try {
        ...
        Stop-CrushRuntime
        Write-Host "  Crush arrete" -ForegroundColor DarkGray
    }
}
...
# RelaunchCommand vaut "setup" — taper `.\crush.ps1` pour lire l'aide aurait
