...
"""Tests du gateway de messagerie unifié Crush.
...
from crush.interfaces.channels.base import ChannelAdapter, IncomingMessage, MessageTarget, Platform

# ── Helpers ───────────────────────────────────────────────────────────────────


def _make_crush_gateway(session_id: str = "sess-abc", response: str = "Bonjour !") -> MagicMock:
...
def test_tous_les_stubs_implementent_linterface() -> None:
    ...
    from crush.interfaces.channels.signal_bot import SignalChannel
    from crush.interfaces.channels.slack_bot import SlackChannel
    from crush.interfaces.channels.whatsapp import WhatsAppChannel
...
@pytest.mark.asyncio
async def test_dispatch_appelle_crush_et_envoie_reponse(tmp_path: Path) -> None:
    """dispatch() appelle crush_gateway.handle() et adapter.send()."""
    from crush.interfaces.channels.gateway import MessagingGateway

    crush_gw = _make_crush_gateway(session_id="sess-1", response="OK !")
    gw = MessagingGateway(crush_gateway=crush_gw, session_map_path=tmp_path / "sessions.json")
    ...
    crush_gw.handle.assert_awaited_once()
    assert len(sent) == 1
    assert sent[0][0] == "OK !"
...
async def test_dispatch_session_id_passe_au_deuxieme_appel(tmp_path: Path) -> None:
    ...
    crush_gw = _make_crush_gateway(session_id="sess-xyz")
    gw = MessagingGateway(crush_gateway=crush_gw, session_map_path=tmp_path / "sessions.json")
    ...
    calls = crush_gw.handle.call_args_list
...
async def test_session_map_persistee_sur_disque(tmp_path: Path) -> None:
    ...
    crush_gw = _make_crush_gateway(session_id="persisted-id")
    path = tmp_path / "map.json"
    gw = MessagingGateway(crush_gateway=crush_gw, session_map_path=path)
...
async def test_session_map_rechargee_depuis_disque(tmp_path: Path) -> None:
    ...
    crush_gw = _make_crush_gateway(session_id="new-sess")
    gw = MessagingGateway(crush_gateway=crush_gw, session_map_path=path)
    ...
    call_kwargs = crush_gw.handle.call_args.kwargs
...
async def test_webhook_plateforme_inconnue() -> None:
    ...
    from crush.interfaces.api.channels import router
