...
from crush.capabilities.tools.memory import MemoryLoadTopicTool, MemorySearchTool
from crush.providers.memory.index import MemoryIndex
from crush.providers.memory.search import VectorIndex, _chunk_text
...
def test_memory_index_add_pointer_existing_section(tmp_path: Path) -> None:
    md = tmp_path / "MEMORY.md"
    initial = "# Index\n\n## Projets actifs\n- crush: `topics/crush.md` — Crush\n\n## Autre\n"
    # encoding explicite : sans lui, Windows écrit en cp1252 et MemoryIndex.read(),
    # qui lit en UTF-8, casse sur le tiret cadratin.
...
def test_agent_build_system_lists_topics_only(tmp_path: Path) -> None:
    ...
    from crush.kernel.settings import settings as _settings
