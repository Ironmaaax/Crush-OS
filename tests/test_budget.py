...
def test_claim_step_premier_worker_gagne() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
        import crush.engine.mission.project_store as ps_mod

        original = ps_mod.WORKSPACE_DIR
        ps_mod.WORKSPACE_DIR = Path(tmpdir)
        try:
            (Path(tmpdir) / "proj1" / ".crush").mkdir(parents=True, exist_ok=True)
...
def test_claim_step_deux_etapes_differentes() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        ps_mod.WORKSPACE_DIR = Path(tmpdir)
        try:
            (Path(tmpdir) / "proj2" / ".crush").mkdir(parents=True, exist_ok=True)
            from crush.engine.mission.project_store import ProjectStore
...
def test_release_step_claim_permet_re_claim() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        ps_mod.WORKSPACE_DIR = Path(tmpdir)
        try:
            (Path(tmpdir) / "proj3" / ".crush").mkdir(parents=True, exist_ok=True)
            from crush.engine.mission.project_store import ProjectStore
...
def _make_project(tmpdir: str, project_id: str = "proj_test", n_steps: int = 3) -> object:
    from crush.engine.mission.schemas import Project, ProjectStatus, Step, StepStatus
...
def test_pause_for_budget_met_running_en_pending() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            (ws / ".crush").mkdir(parents=True, exist_ok=True)

            from crush.engine.mission.project_store import ProjectStore
            from crush.engine.mission.schemas import ProjectStatus, StepStatus
...
def test_is_resumable_faux_si_running() -> None:
    import crush.engine.mission.project_store as ps_mod
    from crush.engine.mission.schemas import ProjectStatus
...
def test_projet_pause_reprise_reprend_etapes_pending() -> None:
    ...
    from crush.engine.mission.schemas import StepStatus
