...
from crush.providers.memory.ingest import (
    CONFIDENCE_EXPLICIT,
...
from crush.providers.memory.schemas import FactStatus, ObservationType, RelationType
...
async def test_decay_policy_par_categorie(kernel: MemoryKernel) -> None:
    ...
    from crush.providers.memory.schemas import DecayPolicy
...
# ── Filtre : l'assistant ne se mémorise pas lui-même ──────────────────────────
#
# RÉGRESSION : la mémoire accumulait « crush communicates_as formel » (vu 4×).
# Le prompt système décrit le caractère de l'assistant à chaque échange ;
# l'extracteur le relisait et en tirait des « faits », qui se renforçaient tout
# seuls et occupaient le contexte sans rien apprendre sur l'utilisateur.


def test_persona_de_l_assistant_ignoree() -> None:
    """Un fait de catégorie `persona` sur l'assistant ne doit pas être stocké."""
    from crush.providers.memory.ingest import MemoryIngest

    ingest = MemoryIngest.__new__(MemoryIngest)
    ingest._assistant_name = "Crush"
    assert ingest._decrit_sa_propre_persona("crush", "persona") is True
    assert ingest._decrit_sa_propre_persona("Crush", "persona") is True
    assert ingest._decrit_sa_propre_persona("assistant", "persona") is True


def test_lecons_de_mission_conservees() -> None:
    """Le module Réflexion écrit ses leçons sous (crush, decided, …) : à garder.

    Le filtre porte sur la CATÉGORIE, pas sur le sujet — sans quoi il
    supprimerait aussi ces enregistrements, qui eux ont de la valeur.
    """
    from crush.providers.memory.ingest import MemoryIngest

    ingest = MemoryIngest.__new__(MemoryIngest)
    ingest._assistant_name = "Crush"
    assert ingest._decrit_sa_propre_persona("crush", "decision") is False
    assert ingest._decrit_sa_propre_persona("crush", "skill") is False


def test_persona_de_l_utilisateur_conservee() -> None:
    """La façon de communiquer de l'UTILISATEUR reste un fait utile."""
    from crush.providers.memory.ingest import MemoryIngest

    ingest = MemoryIngest.__new__(MemoryIngest)
    ingest._assistant_name = "Crush"
    assert ingest._decrit_sa_propre_persona("max", "persona") is False
