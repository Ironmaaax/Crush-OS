...
from crush.engine.mission.governance import Governance
from crush.engine.mission.project_store import ProjectStore
from crush.engine.mission.schemas import (
    Project,
    ProjectStatus,
    Step,
    StepStatus,
...
from crush.engine.mission.verifier import VerificationResult, Verifier
from crush.engine.mission.worker_agent import WorkerAgent
...
def test_persistance_step_roundtrip(tmp_path: Path) -> None:
    ...
    with patch("crush.engine.mission.project_store.WORKSPACE_DIR", tmp_path):
        store = ProjectStore()
...
def test_persistance_projet_ancien_format_compat(tmp_path: Path) -> None:
    ...
    with patch("crush.engine.mission.project_store.WORKSPACE_DIR", tmp_path):
        ...
        (workspace / ".crush").mkdir(parents=True)
        legacy_state = {
            "id": project_id,
            "title": "Vieux projet",
        ...
        (workspace / ".crush" / "state.json").write_text(
