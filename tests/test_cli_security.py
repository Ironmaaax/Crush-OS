...
from crush.capabilities.tools.cli import ExecuteCLITool
...
# ── Faille 1 : osascript — désormais refusé en amont de l'approbation ─────────
#
# Ces tests attendaient qu'`osascript` soit exécutable après confirmation. Ils
# datent du déploiement macOS d'origine. L'assistant tourne maintenant sur une
# Raspberry Pi sous Debian : `osascript` n'y existe pas, et l'autoriser ne
# produisait qu'un « command not found » opaque après avoir fait valider un
# risque à l'utilisateur pour rien.
#
# Il est donc refusé par l'allowlist, EN AMONT du mécanisme d'approbation. La
# garde est plus stricte qu'avant, pas plus laxiste — et le message renvoie
# vers `remote_pc`, le seul outil qui atteigne réellement la machine de
# l'utilisateur. La classification d'`osascript` comme sensible reste vérifiée
# par les tests unitaires de `_requires_approval` ci-dessus, inchangés.


async def test_osascript_refuse_par_l_allowlist(tool: ExecuteCLITool) -> None:
    """osascript n'est plus exécutable : binaire macOS absent de cette machine."""
    result = await tool.execute(command="osascript -e 'do shell script \"rm -f /tmp/x\"'")
    assert result.is_error
    assert "osascript" in result.content
    assert "remote_pc" in result.content, "le refus doit indiquer l'outil qui, lui, agit sur le PC"


async def test_osascript_chemin_absolu_refuse_aussi(tool: ExecuteCLITool) -> None:
    """Le chemin absolu ne contourne pas l'allowlist : la comparaison porte sur le nom."""
    result = await tool.execute(command="/usr/bin/osascript -e 'beep'")
    assert result.is_error
    assert "osascript" in result.content


async def test_binaire_sensible_confirme_est_execute(
    tool: ExecuteCLITool,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """confirmed=True lance réellement le sous-processus sur un binaire sensible.

    Porté d'`osascript` (macOS, retiré de l'allowlist) vers `shutdown`, qui
    existe sur Debian et reste sous approbation. C'est le seul test qui couvre
    le chemin « approbation accordée → sous-processus lancé » : le convertir
    plutôt que le supprimer préserve cette couverture.
    """
    mock_settings = MagicMock()
    mock_settings.allow_unsandboxed_exec = False
    monkeypatch.setattr("crush.capabilities.tools.cli.settings", mock_settings)
    ...
    with patch("asyncio.create_subprocess_exec", AsyncMock(return_value=mock_proc)) as mock_exec:
        result = await tool.execute(command="shutdown -h now", confirmed=True)
...
async def test_sips_refuse_et_renvoie_vers_convert(tool: ExecuteCLITool) -> None:
    """`sips` est l'outil d'image de macOS. Sur Debian, c'est `convert`.

    Le refus doit nommer le remplaçant : sans cela le modèle réessaie la même
    commande, ou conclut à tort qu'il ne sait pas retailler une image.
    """
    result = await tool.execute(command="sips -z 800 600 input.jpg --out output.jpg")
    assert result.is_error
    assert "convert" in result.content
