...
def _sans_horodatage(contenu: str) -> list[str]:
    """Lignes du miroir, privées de celle qui porte la date de génération."""
    return [ligne for ligne in contenu.splitlines() if not ligne.startswith("_Mis à jour :")]
...
def test_edition_manuelle_ecrasee_a_la_regeneration(tmp_path: Path, kernel: MemoryKernel) -> None:
    ...
    # Égalité SAUF l'horodatage : le miroir date sa dernière ligne à la
    # seconde, et deux exports qui tombent de part et d'autre d'une seconde
    # diffèrent légitimement. L'ancienne comparaison stricte rendait ce test
    # intermittent — un test qui clignote finit par être ignoré, et emporte
    # avec lui la confiance dans toute la suite.
...
def test_needs_review_fichier_separe(tmp_path: Path, kernel: MemoryKernel) -> None:
    ...
    mirror = MemoryMirror(kernel, tmp_path / "mirror")
    report = mirror.export()
    assert "crush/needs-review.md" in report.files_written
    content = (tmp_path / "mirror/crush/needs-review.md").read_text(encoding="utf-8")
...
def test_uncertain_beliefs_separes_par_confidence(tmp_path: Path, kernel: MemoryKernel) -> None:
    ...
    mirror = MemoryMirror(kernel, tmp_path / "mirror")
    report = mirror.export()
    assert "crush/persona.md" in report.files_written
    assert "crush/uncertain-beliefs.md" in report.files_written

    persona_md = (tmp_path / "mirror/crush/persona.md").read_text(encoding="utf-8")
    uncertain_md = (tmp_path / "mirror/crush/uncertain-beliefs.md").read_text(encoding="utf-8")
