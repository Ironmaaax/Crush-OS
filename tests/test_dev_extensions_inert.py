...
Garantie : sans extension dev liée, Crush charge skills/installed/ exactement
...
from crush.capabilities.skills.dev_extensions import iter_dev_skills_and_presets, mount_dev_views
from crush.capabilities.skills.registry import SKILLS_INSTALLED_DIR, SkillRegistry
...
def test_a_zone_dev_absente_charge_uniquement_installed(
    ...
    """HOME redirigé vers un tmp_path SANS .crush/extensions/dev → la zone est
    inexistante. load_all() doit charger exactement skills/installed/."""
    monkeypatch.setenv("CRUSH_DEV_EXTENSIONS_DIR", str(tmp_path / "absent"))
...
def test_b_zone_dev_vide_charge_uniquement_installed(
    ...
    monkeypatch.setenv("CRUSH_DEV_EXTENSIONS_DIR", str(dev_root))
