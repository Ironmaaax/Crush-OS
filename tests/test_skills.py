...
"""Tests de la boucle d'apprentissage de skills Crush.
...
async def test_improve_skill_met_a_jour_les_fichiers(tmp_path: Path) -> None:
    ...
    with patch("crush.capabilities.skills.synthesizer.SKILLS_INSTALLED_DIR", tmp_path):
...
async def test_skill_synthetise_chargeable_par_le_registry(tmp_path: Path) -> None:
    ...
    registry._skills = {}
    with patch("crush.capabilities.skills.registry.SKILLS_INSTALLED_DIR", tmp_path):
        registry.load_all()
...
def test_export_to_standard_produit_skill_md_valide(tmp_path: Path) -> None:
    ...
    from crush.capabilities.skills.standard import AgentSkillsAdapter

    with patch("crush.capabilities.skills.standard.SKILLS_INSTALLED_DIR", tmp_path):
...
def test_round_trip_export_import(tmp_path: Path) -> None:
    ...
    # Export Crush → SKILL.md
    with patch("crush.capabilities.skills.standard.SKILLS_INSTALLED_DIR", tmp_path):
        skill_md = AgentSkillsAdapter.export_to_standard(skill_name)

    # Import SKILL.md → Crush (dans un dossier séparé)
    import_dir = tmp_path / "imported"
    import_dir.mkdir()
    with patch("crush.capabilities.skills.standard.SKILLS_INSTALLED_DIR", import_dir):
...
async def test_skill_create_tool_succes(tmp_path: Path) -> None:
    ...
    from crush.capabilities.skills.synthesizer import SkillSynthesizer
    from crush.capabilities.tools.skills import SkillCreateTool
...
async def test_skill_list_tool_vide() -> None:
    ...
    from crush.capabilities.tools.skills import SkillListTool

    mock_registry = MagicMock()
    mock_registry.list_installed.return_value = []

    tool = SkillListTool()
    with patch("crush.capabilities.tools.skills.skill_registry", mock_registry):
        result = await tool.execute()
...
async def test_skill_improve_tool_skill_absent() -> None:
    ...
    from crush.capabilities.skills.synthesizer import SkillSynthesizer
    from crush.capabilities.tools.skills import SkillImproveTool
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        with patch("crush.capabilities.skills.synthesizer.SKILLS_INSTALLED_DIR", Path(tmpdir)):
            result = await tool.execute(
