...
from crush.kernel.bundle_download import (
    BUNDLE_ZIP_BYTES,
    BUNDLE_ZIP_URL,
    _zip_member_dest,
...
def test_start_bundle_download_when_already_present() -> None:
    ...
    with (
        patch("crush.kernel.bundle_download.sys.platform", "win32"),
        patch("crush.kernel.bundle_download.bundle_available", return_value=True),
    ):
...
def test_start_bundle_download_refuse_hors_windows() -> None:
    with patch("crush.kernel.bundle_download.sys.platform", "linux"):
        resp = start_bundle_download()
...
def test_get_download_status_idle_by_default() -> None:
    with patch(
        "crush.kernel.bundle_download._state",
        {
            "running": False,
            "phase": "idle",
            "percent": 0,
        ...
        with patch("crush.kernel.bundle_download.bundle_available", return_value=False):
            with patch("crush.kernel.bundle_download.prerequisites_status", return_value={}):
...
def test_get_download_status_returns_done_once() -> None:
    with patch(
        ...
        with (
            patch("crush.kernel.bundle_download.bundle_available", return_value=True),
            patch(
                "crush.kernel.bundle_download.prerequisites_status",
