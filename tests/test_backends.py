...
from crush.engine.mission.backend_factory import get_backend
from crush.engine.mission.backends.base import ExecutionBackend
...
from crush.engine.mission.backends.rpc import ScriptRPCRunner, _build_stub
from crush.engine.mission.backends.ssh import SSHBackend
from crush.kernel.backends import BackendsConfig, BackendType, SSHConfig
...
class TestGetBackend:
    ...
    def test_auto_avec_docker_executor_retourne_docker(self, tmp_path: Path) -> None:
        executor = MagicMock()
        with patch(
            "crush.engine.mission.backend_factory.load_backends_config",
            return_value=BackendsConfig(),
        ):
            with patch("crush.engine.mission.backend_factory.settings") as mock_settings:
    ...
    def test_ssh_sans_host_retourne_none(self, tmp_path: Path) -> None:
        ...
        with patch("crush.engine.mission.backend_factory.load_backends_config", return_value=cfg):
    ...
    def test_ssh_avec_config_retourne_ssh_backend(self, tmp_path: Path) -> None:
        cfg = BackendsConfig(
            ...
            ssh=SSHConfig(host="host.example.com", user="crush"),
        )
        with patch("crush.engine.mission.backend_factory.load_backends_config", return_value=cfg):
            backend = get_backend(str(tmp_path))
...
class TestDockerBackend:
    ...
    async def test_is_available_respecte_docker_enabled(self) -> None:
        executor = MagicMock()
        backend = DockerBackend(executor)
        with patch("crush.engine.mission.backends.docker.settings") as mock_settings:
            mock_settings.docker_enabled = False
            with patch(
                "crush.engine.mission.docker_executor.DockerExecutor.is_available",
...
class TestRefusSansBackendSur:
    ...
    async def test_local_refuse_sans_optin(self, tmp_path: Path) -> None:
        ...
        with patch("crush.engine.mission.backends.local.settings") as mock_settings:
...
class TestSpawnSubagent:
    ...
    async def test_spawn_retourne_resume(self) -> None:
        from crush.capabilities.tools.subagent import SpawnSubagentTool
...
class TestScriptRPC:
    ...
    async def test_script_appelle_outil_rpc(self, tmp_path: Path) -> None:
        ...
        async def fake_execute(command: str, timeout: int = 60) -> dict:  # noqa: ASYNC109
            rpc_dir_path = tmp_path / ".crush_rpc"
        ...
        runner = ScriptRPCRunner(mock_backend, mock_registry, tmp_path)
        result = await runner.run("import crush_tools; print(crush_tools.weather(city='Lyon'))")
    ...
    async def test_outil_non_autorise_retourne_erreur_rpc(self, tmp_path: Path) -> None:
        ...
        runner = ScriptRPCRunner(mock_backend, mock_registry, tmp_path)
        await runner.run("import crush_tools")
...
class TestWorkerCLIRouting:
    ...
    async def test_route_vers_docker_si_dispo(self, tmp_path: Path) -> None:
        from crush.engine.mission.worker_cli import WorkerCLITool
        ...
        with patch(
            ...
            with patch("crush.engine.mission.backend_factory.settings") as s:
