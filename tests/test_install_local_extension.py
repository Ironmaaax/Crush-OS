...
def _fake_crush_skills(tmp_path: Path, *, validator_rc: int | None) -> Path:
    """Crée un faux repo `crush-skills/` minimal.
...
def _run(args: list[str], dev_root: Path) -> subprocess.CompletedProcess:
    env = {**os.environ, "CRUSH_DEV_EXTENSIONS_DIR": str(dev_root)}
...
def test_refus_si_validate_catalog_absent(tmp_path: Path) -> None:
    repo = _fake_crush_skills(tmp_path, validator_rc=None)
...
def test_refus_si_validate_catalog_rc_non_zero(tmp_path: Path) -> None:
    repo = _fake_crush_skills(tmp_path, validator_rc=1)
    src = _write_skill(repo / "skills", "x", "skill")
    dev = tmp_path / "dev"
...
def test_install_cree_symlink(tmp_path: Path, kind: str, dest_subdir: str) -> None:
    repo = _fake_crush_skills(tmp_path, validator_rc=0)
