...
def secret_sentinels(monkeypatch: pytest.MonkeyPatch) -> dict[str, str]:
    ...
    dans `crush/kernel/__init__.py`). Les call-sites consomment le binding
    capturé via `from crush.kernel.settings import settings` ; tous pointent
...
def test_anthropic_provider_passes_str(secret_sentinels: dict[str, str]) -> None:
    ...
    from crush.providers.llm import api as mod
...
def test_notion_widgets_headers(secret_sentinels: dict[str, str]) -> None:
    ...
    from crush.interfaces.api.widgets import _notion_headers
...
def test_spotify_basic_auth_uses_str(secret_sentinels: dict[str, str]) -> None:
    ...
    from crush.capabilities.tools.spotify_auth import _basic_auth
...
def test_elevenlabs_tts_header(secret_sentinels: dict[str, str]) -> None:
    ...
    from crush.providers.audio import tts as mod
...
def test_deezer_callback_secret_param(secret_sentinels: dict[str, str]) -> None:
    ...
    from crush.interfaces.api import deezer as mod
...
def test_api_auth_verify_token_drives_real_middleware(
    ...
    from crush.engine.auth import verify_api_token
...
def test_api_auth_verify_token_rejects_wrong_bearer(
    ...
    # Depuis le passage au modèle cookie, `verify_api_token` inspecte aussi ces
    # deux porteurs d'identité. Un MagicMock les rendrait « présents mais
    # illisibles » ; on les fournit vides pour n'exercer QUE le chemin Bearer.
    fake_req.cookies = {}
...
def test_globe_config_keys(secret_sentinels: dict[str, str]) -> None:
    ...
    from crush.interfaces.api.globe import get_config
