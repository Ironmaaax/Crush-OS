...
def _enable_files_permission() -> Generator[None, None, None]:
    ...
    """
    from crush.kernel.permissions import permissions
...
async def test_weather_tool_success() -> None:
    ...
    with patch("crush.capabilities.tools.weather.httpx.AsyncClient", return_value=mock_instance):
...
async def test_read_file_success(tmp_path: Path) -> None:
    from crush.capabilities.tools.filesystem import ReadFileTool
...
async def test_read_file_too_large(tmp_path: Path) -> None:
    from crush.capabilities.tools.filesystem import _MAX_FILE_SIZE, ReadFileTool
...
async def test_find_files_success(tmp_path: Path) -> None:
    from crush.capabilities.tools.filesystem import FindFilesTool
