...
crush-OS est un serveur FastAPI async, self-hosted, multi-LLM (Anthropic principal, Mistral/Gemini/Ollama en fallback), avec pipeline vocal LiveKit. Modules pertinents pour ce CDC :
...
**6.7 — Miroir Markdown UNIDIRECTIONNEL (`memory/mirror.py`).** SQLite → Markdown, **lecture seule côté humain**. Génère ex. `user/preferences.md`, `user/projects.md`, `user/goals.md`, `crush/persona.md`, `crush/uncertain-beliefs.md`. Lisible (compatible Obsidian) pour inspecter/visualiser. **Aucune édition directe des .md ne modifie la mémoire.** Pour corriger un souvenir, l'utilisateur passe par une commande Crush qui crée un `Event` `human_correction` (catégorie `memory_correction`) → la DB se met à jour. Le bidirectionnel propre est hors scope MVP.
...
**10.2 — Command Center.** Vue de toutes les initiatives/missions en cours avec objectifs, budgets, permissions, états, heartbeat, coûts. Crush ne "fait pas des trucs" : il gère des workstreams.
...
### Definition of Done
Crush propose une initiative pertinente non sollicitée, la gère via le Command Center avec budget et permissions, et un Curator nocturne produit un rapport de maintenance exploitable.
...
- **Persona adaptative** : ton, humour, niveau de détail, types de conseils préférés. Modifiable par Crush (facts de catégorie `persona`) **mais avec validation**, et exposée dans le miroir Markdown (`crush/persona.md`) pour inspection.
