# CDC — Refonte architecturale crush-OS
...
6. **Compatibilité ascendante pendant la migration.** Les commandes `crush run`, `crush voice`, `crush eclosion`, `make start` doivent fonctionner à la fin de chaque phase.
7. **En cas de doute : STOP et question.** Une question coûte 2 minutes, un mauvais choix architectural coûte une semaine.
8. **Précondition de séquencement produit.** La refonte démarre depuis un commit où la feature produit en cours (Phase 6 — Capability Engine, cf. `CDC_crush_evolution.md`) est **stable, mergée et taggée `pre-reorg`**. On ne gèle JAMAIS en milieu de feature : du code à moitié intégré sous une migration de cette ampleur est ingérable. Si la feature en cours n'est pas stabilisable rapidement, la refonte attend.
...
uv run python -c "import crush"     # à partir de la phase B
```
...
| **[LOCAL]** (ta machine, vraies données) | continuité des données (A8/B8 : faits, tokens, 8 skills, faces), smoke `--real` (clé Anthropic), tests manuels de fin de phase, `crush voice` réel | Dépend de TON état runtime, irreproductible en CI |
...
│   │   ├── errors.py            (hiérarchie d'exceptions Crush)
...
├── tests/
│   ├── unit/                    (miroir de src/crush/)
│   └── integration/
├── docs/
│   ├── CDC_crush_evolution.md      (déplacé depuis la racine)
│   ├── CDC_refonte_architecture.md  (ce document)
│   ├── INTEGRATION-DONE.md          (déplacé)
│   └── migration/BACKLOG.md
├── main.py                  # shim de compat : `from crush.app import main; main()`
├── voice_agent.py           # shim de compat équivalent
├── crush                   # CLI inchangée (chemins mis à jour)
├── pyproject.toml           # package `crush-os`, src-layout
└── …
```
...
```
RÈGLE 1 — kernel n'importe RIEN de crush.* (stdlib + pydantic uniquement).
RÈGLE 2 — providers/ et capabilities/ n'importent QUE crush.kernel.
RÈGLE 3 — engine/ n'importe QUE crush.kernel ; il reçoit providers,
...
1. **Créer `scripts/migration/audit_imports.sh`** — le thermomètre de toute la migration :
   ...
   PKGS="core|memory|tools|skills|agent|api|proactive|channels|background|llm|config|audio|vision|kernel|crush"
...
2. **Créer `scripts/migration/snapshot_routes.py`** — preuve mécanique de l'identité des routes HTTP à travers toute la migration :
   ...
   try:
       from crush.app import app          # à partir de la Phase B
   except ImportError:
...
3. **Créer `kernel/`** (à la racine pour l'instant — il migrera dans `src/crush/` en Phase B) :
   - `kernel/errors.py` : hiérarchie `CrushError` → `LLMError`, `MemoryError_`, `ToolError`, `SkillError`, `BudgetExceeded`, `PermissionDenied`. Recenser les exceptions ad hoc existantes (`grep -rn "class.*Error" --include="*.py" core/ llm/ memory/ tools/`) et les faire hériter de cette hiérarchie **sans changer leur comportement**.
...
## 4. PHASE B — Migration vers le layout `src/crush/`

**Objectif :** déplacer tout le code applicatif dans `src/crush/` selon l'arborescence cible (§2.1), réécrire tous les imports, rendre le package installable. **Aucun changement de logique — uniquement des déplacements et des renommages d'imports.**
...
| Source (racine) | Destination (`src/crush/`) |
|---|---|
| `kernel/` | `kernel/` |
| `llm/` | `providers/llm/` |
...
| `main.py` | `src/crush/app.py` + shim `main.py` racine |

Restent à la racine : `prompts/`, `notices/`, `scripts/`, `tests/`, `crush` (CLI), `Makefile`, `docs/`.

### B.2 Tâches

1. **Déplacer avec `git mv`** (préservation de l'historique — JAMAIS de copier-coller).
2. **Réécrire les imports** par passes successives de find/replace, package par package, dans cet ordre : kernel → providers → capabilities → engine → interfaces. Après CHAQUE package migré : `uv run python -c "import crush.<package>"` + commit.
   Mapping : `from core.X import` → `from crush.engine.X import` ; `from llm.` → `from crush.providers.llm.` ; etc.
   **Imports dynamiques inclus** : les namespaces sous forme de chaîne (`__import__("tools.weather")`, `import_module("...")`) échappent aux greps `^from` — les réécrire aussi (`__import__("crush.capabilities.tools.weather")`). Source de vérité : la section « imports dynamiques » d'`audit_imports.sh`. Baseline : 14+ occurrences dans `voice_agent.py`.
...
uv pip install -e . && uv run python -c "import crush; import crush.app" && echo "GATE B2 ✅"
...
git log --follow --oneline src/crush/engine/gateway.py | wc -l   # doit être > 1
...
1. **Créer `src/crush/bootstrap.py`** — l'unique composition root :
   ```python
   @dataclass
   class Container:
       settings: Settings
...
8. **Le process voix est un SECOND composition root — pas un simple fichier déplacé.** État actuel de `interfaces/voice/agent.py` (ex `voice_agent.py`) : il construit son graphe en direct — 14+ `__import__("tools.X")` en lambdas, `Path(__file__)`, `.env` chargé localement, `AgentSession` instancié à la main — dans un **process séparé** (`crush voice` / `make voice`), sans passer par aucun composition root. Après l'inversion de dépendance, il serait soit cassé, soit en contradiction frontale avec l'architecture. Décision architecturale (ferme) :
...
grep -rn "^from crush\.\(providers\|capabilities\|interfaces\)" --include="*.py" src/crush/engine/ | grep -v TYPE_CHECKING ; test $? -eq 1 && echo "GATE C1 ✅"

# GATE C2 — providers n'importent que kernel
grep -rn "^from crush\.\(engine\|capabilities\|interfaces\)" --include="*.py" src/crush/providers/ | grep -v TYPE_CHECKING ; test $? -eq 1 && echo "GATE C2 ✅"

# GATE C3 — capabilities n'importent que kernel
grep -rn "^from crush\.\(engine\|providers\|interfaces\)" --include="*.py" src/crush/capabilities/ | grep -v TYPE_CHECKING ; test $? -eq 1 && echo "GATE C3 ✅"
...
test $(wc -l < src/crush/app.py) -lt 330 && echo "GATE C5 ✅"

# GATE C6 — bootstrap construit le graphe complet sans réseau
uv run python -c "from crush.bootstrap import build; c = build(); print(type(c).__name__)" && echo "GATE C6 ✅"
...
grep -rn "noqa: F401" --include="*.py" src/crush/ | grep -v "capabilities/skills/_loader.py" ; test $? -eq 1 && echo "GATE C9 ✅"
...
grep -cE "__import__\(" src/crush/interfaces/voice/agent.py | grep -q "^0$" && echo "GATE C10b ✅ (zéro __import__ résiduel dans la voix)"
```
...
- `crush voice` lancé réellement si LiveKit dispo : un échange vocal avec appel d'outil (météo) — c'est le seul test qui valide le Container voix en conditions réelles.
...
grep -rn "^from crush\.\(providers\|capabilities\|interfaces\)" --include="*.py" src/crush/engine/ | grep -v TYPE_CHECKING ; test $? -eq 1 && echo "GATE D1 ✅"

# GATE D2 — events.md exhaustif : chaque dataclass d'événement y figure
python - << 'EOF'
import re, pathlib
src = pathlib.Path("src/crush/kernel/events.py").read_text()
...
grep -rn "from crush.interfaces" --include="*.py" src/crush/capabilities/ src/crush/providers/ src/crush/engine/ ; test $? -eq 1 && echo "GATE E1 ✅"

# GATE E2 — convention de nommage api unifiée + PREUVE que les URL n'ont pas bougé
ls src/crush/interfaces/api/ | grep "^http_" ; test $? -eq 1 && echo "GATE E2a ✅"
uv run python scripts/migration/snapshot_routes.py > /tmp/routes.now.txt
...
find src/crush/interfaces/api -name "*.py" | xargs wc -l | awk '$1 > 600 && $2 != "total" {print; f=1} END {exit f}' && echo "GATE E3 ✅"
...
1. **import-linter** — le contrat d'architecture exécutable :
   ...
   type = "layers"
   layers = [
       "crush.interfaces | crush.app | crush.bootstrap",
       "crush.engine",
       "crush.providers | crush.capabilities | crush.analytics | crush.hardware",
       "crush.kernel",
   ]
   ...
   forbidden_modules = ["crush.engine", "crush.providers", "crush.capabilities", "crush.interfaces"]
   ```
...
3bis. **Type-checker — rendre les Protocols réellement contraignants.** Sans type-checker, `kernel/contracts.py` est de la documentation, pas un contrat : les Protocols Python sont structurels et ne sont JAMAIS vérifiés au runtime. Attention au piège : `mypy src/crush/providers` seul ne prouve PAS la conformité — mypy ne compare les signatures qu'au point d'assignation typée. Mécanisme imposé :
   - Créer `tests/unit/kernel/test_contracts_conformance.py` : une assignation typée explicite **par implémentation** :
     ```python
     from crush.kernel.contracts import LLMProvider, MemoryStore, ToolRegistry
     ...
     ```
   - mypy en CI et Makefile, **scopé** : `uv run mypy src/crush/kernel tests/unit/kernel/test_contracts_conformance.py` — PAS de mypy strict sur les 46k lignes (chantier séparé, → BACKLOG).
...
4. **Réorganisation des tests** : `tests/unit/` en miroir de `src/crush/` (`tests/unit/engine/`, `tests/unit/providers/memory/`, …), `tests/integration/` pour les tests qui touchent disque/réseau/Docker. Marqueurs pytest (`@pytest.mark.integration`) + config : `pytest` seul = unit only, `pytest -m integration` = le reste. Objectif : suite unitaire **< 30 s**.
5. **Hygiène racine et assets** :
   - Supprimer `ui/static/*.old.*` (5 fichiers) — `git rm`.
   - Déplacer `CDC_crush_evolution.md`, `INTEGRATION-DONE.md`, ce CDC → `docs/`.
   - Compresser `Favicon.png` (cible < 30 Ko) et `Cover_Crush_Github.png` (cible < 300 Ko, ou WebP).
...
6. **Identité** : `crush-os` partout (pyproject ✅ en phase B, vérifier README, CLI `--version`, UI footer).
...
uv run mypy src/crush/kernel tests/unit/kernel/test_contracts_conformance.py && echo "GATE F1bis-a ✅"
uv run python -c "from crush.bootstrap import build; build()" && echo "GATE F1bis-b ✅ (asserts isinstance au boot)"
...
ls *.old.* CDC_crush_evolution.md INTEGRATION-DONE.md 2>/dev/null ; test $? -ne 0 && echo "GATE F4 ✅"

# GATE F5 — poids des assets
test $(stat -c%s ui/static/favicon.png 2>/dev/null || stat -c%s src/crush/interfaces/ui/static/favicon.png) -lt 30720 && echo "GATE F5 ✅"
