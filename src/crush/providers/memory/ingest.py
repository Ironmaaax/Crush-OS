...
from crush.kernel.events import EventBus, MemoryIngested
from crush.kernel.vocab import CATEGORIES, PREDICATES
from crush.providers.llm.base import LLMProvider
from crush.providers.memory.kernel import MemoryKernel, _new_id, normalize
...
def _build_prompt(content: str, source: str, name: str = "Max") -> str:
    ...
    return (
        ...
        f"- subject est TOUJOURS '{name}' ou une entité de SON monde "
        "(un projet, une personne, un lieu). N'extrais JAMAIS de fait dont le "
        "sujet est l'assistant lui-même : sa personnalité vient de son prompt, "
        "pas de la mémoire, et l'y stocker pollue le contexte.\n"
...
class MemoryIngest:
    ...
    def __init__(
        ...
        # Sert au filtre qui empeche l'assistant de se memoriser lui-meme.
        self._assistant_name = assistant_name
        # Compteur d'appels au LLM arbitre (étape 2 du matcher v2).
        self.arbiter_calls = 0

    def _decrit_sa_propre_persona(self, subject: str, category: str) -> bool:
        """Vrai si le fait ne fait que redire la personnalite de l'assistant.

        On ne filtre PAS tout fait dont le sujet est l'assistant : le module
        Reflexion enregistre legitimement ses lecons de mission sous
        (`crush`, `decided`, ..., categorie `decision`), et elles ont de la
        valeur.

        Ce qui polluait, c'etait la seule categorie `persona` : le prompt
        systeme decrit le caractere a chaque echange, l'extracteur le relisait
        et en tirait des « faits » (« crush communicates_as formel », vu 4x)
        qui se renforcaient tout seuls et occupaient le contexte.
        """
        if normalize(category) != "persona":
            return False
        s = normalize(subject)
        return s in {normalize(self._assistant_name), "crush", "assistant", "l'assistant"}
    ...
    async def ingest(
        ...
        for cand in candidates:
            # Garde-fou : l'assistant ne se memorise pas lui-meme. Le prompt le
            # demande deja, mais un modele s'en ecarte — on avait accumule des
            # faits « crush communicates_as formel » (vus 4x), qui occupaient le
            # contexte a chaque echange et se renforcaient tout seuls, puisque le
            # prompt systeme les redecrivait a chaque fois.
            if self._decrit_sa_propre_persona(cand.subject, cand.category):
                logger.debug(
                    "Fait ignoré — l'assistant redécrit sa propre persona : {} {} {}",
