...
# `sounddevice` vit dans l'extra `local-audio` : il ouvre le micro de la machine
# qui héberge l'assistant. Sur un serveur headless (Raspberry Pi 24/7), ce micro
# n'existe pas — c'est celui du navigateur client qui sert, via LiveKit. L'import
# est donc différé jusqu'à `start()`, pour que le module reste importable sans
# l'extra installé.
...
class ClapDetector:
    ...
    async def start(self) -> None:
        ...
        suppose un micro physique sur la machine qui héberge Crush. On le signale
        ...
        try:
            ...
            # OSError : PortAudio absent du système (libportaudio2 non installé).
            logger.warning(
                "ClapDetector désactivé : extra `local-audio` non installé ({}). "
                "Installe-le avec `uv sync --extra local-audio` si la machine a un "
                "micro, sinon mets CLAP_DETECTION_ENABLED=false.",
                exc,
            )
            self._running = False
            return
