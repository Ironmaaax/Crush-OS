...
UNIQUE point d'instanciation du graphe d'objets de Crush. AUCUNE logique
...
from crush.capabilities.tools.browser import BrowserTool
from crush.capabilities.tools.calendar import CalendarCreateTool, CalendarListTool
from crush.capabilities.tools.capability import ReportMissingCapabilityTool
from crush.capabilities.tools.cli import CLIRunnerTool, ExecuteCLITool
...
from crush.capabilities.tools.notion import NotionTasksTool
...
from crush.capabilities.tools.show_view import ShowViewTool
...
from crush.engine.approval_checker import ApprovalChecker
from crush.engine.background.notifications import (
    NotificationQueue,
    ProactiveQueue,
    set_proactive_queue,
)
...
from crush.engine.mission.capability_engine import CapabilityEngine, Whitelist
from crush.engine.mission.orchestrator import ProjectOrchestrator
from crush.engine.mission.project_manager import ProjectManager
from crush.engine.mission.project_store import ProjectStore
from crush.engine.mission.reflexion import Reflexion
from crush.engine.proactive.command_center import CommandCenter
from crush.engine.proactive.context_builder import ContextBuilder
from crush.engine.proactive.curator import Curator
from crush.engine.proactive.engine import ProactiveEngine
from crush.engine.proactive.executor import InitiativeExecutor
...
from crush.engine.tracking import UsageEntry, UsageTracker
from crush.kernel.approval import set_approval_checker
from crush.kernel.events import (
...
from crush.providers.llm.factory import (
    create_background_llm,
    create_reflective_llm,
    create_voice_llm,
    get_llm_provider,
)
...
class Container:
    ...
    # historique (crush.capabilities.skills.registry.skill_registry).
...
def build(
    ...
    memory_kernel = MemoryKernel(memory_dir / "crush_memory.db")
    ...
    if llm_override is not None:
        ...
        reflective_llm: LLMProvider | None = None
    else:
        ...
        reflective_llm = create_reflective_llm(tracker=tracker)

    # ── 4ter. Providers L1 — TTS (singleton module-level — set_tracker post-construction) ─

    tts_engine.set_tracker(tracker)

    # ── 4quater. Providers L1 — STT ────────────────────────────────────────
    # Construction sans réseau ni chargement de modèle (GATE C6) : le modèle
    # local n'est chargé qu'à la première transcription, et le client HTTP
    # OpenAI est créé par requête.
    ...
    tool_registry.register(
        ...
        # Pilotage du poste de l'utilisateur : le registre est un singleton
        # kernel, alimenté par le canal WebSocket des agents distants.
        RemotePCTool(),
    )
    ...
    skill_lifecycle = SkillLifecycle(db_path=memory_dir / "crush_memory.db")
    skill_lab = SkillLab(
        kernel=memory_kernel,
    ...
    voice_agent = Agent(
        settings=settings,
        llm=voice_llm,
        # Jumeau raisonnant : `voice_llm` repond sans reflechir pour tenir la
        # latence, ce qui dessert les questions de fond. L'Agent bascule sur
        # celui-ci pour les seules questions qui portent une marque de
        # raisonnement (cf. engine/reflection.py). None = voie rapide partout.
        llm_reflexion=reflective_llm,
        # Prompt vocal dedie : le prompt bureau pese ~7 800 tokens, dont le
        # prefill s'ecoule avant le premier mot. Mesure sur le Pi : 0,8 s pour
        # un appel nu contre 6,9 s a travers le Gateway, meme modele.
        prompt_path=_VOICE_PROMPT_PATH,
        memory_index=memory_index,
        topic_store=topic_store,
        tool_registry=tool_registry,
        user_prefs_path=user_prefs_path,
    ...
    initiative_store = InitiativeStore()
    # Enregistre ici, et pas en section outils : le magasin n'existe qu'a
    # partir de cette ligne. Sans cet outil, les suggestions proactives
    # n'etaient arbitrables que depuis /dashboard — une page que l'interface
    # mobile ne mentionne nulle part, alors que la voix est l'usage principal.
    tool_registry.register(InitiativesTool(store=initiative_store))
    # Les deux agents sont construits avant le magasin : on le leur passe
    # ici. Sans lui, la ligne « suggestions en attente » du prompt reste
    # muette et les initiatives ne remontent jamais dans la conversation.
    agent._initiative_store = initiative_store
    voice_agent._initiative_store = initiative_store
    ...
    proactive_engine = ProactiveEngine(
        ...
        store=initiative_store,
        interval_minutes=settings.proactive_interval_minutes,
    )
    ...
    scheduler = Scheduler(
        ...
        skill_lab=skill_lab,
        curator=curator,
        # Pour que l'interruption des rappels calendrier remonte dans la
        # conversation. Diffusee sur le bus, elle n'atteindrait que les
    ...
    from crush.kernel import contracts as _contracts
...
def _make_budget_callback(budget: BudgetGuard) -> Callable[[UsageEntry], None]:
    """Câble UsageTracker.track() → BudgetGuard pour les coûts mission ET globaux.
    ...
    def callback(entry: UsageEntry) -> None:
        ...
        # Alerte de plafond global. `track()` est synchrone et la publication
        # sur le bus ne l'est pas : on programme la vérification sur la boucle
        # en cours. Hors boucle (tests, scripts), il n'y a personne pour
        # recevoir l'alerte — on s'abstient plutôt que de lever.
        try:
            boucle = asyncio.get_running_loop()
        except RuntimeError:
            return
        boucle.create_task(budget.noter_depense(entry.cost_usd))
...
def _wire_events(
    *,
    ...
    async def _on_budget_threshold(event: BudgetThresholdReached) -> None:
        ...
        # L'alerte « approche du plafond » ne produisait qu'un événement
        # d'interface, invisible depuis le téléphone et muette à l'oral — donc
        # rien, en pratique. Elle passe désormais par les notifications, que
        # l'assistant glisse en fin de réponse.
        pct = round(event.ratio * 100)
        if event.scope == "global":
            notifications.add(
                f"Budget mensuel atteint à {pct} % — au-delà, le fournisseur coupera."
                if is_hard_stop
                else f"Budget mensuel consommé à {pct} %."
            )
        elif is_hard_stop:
