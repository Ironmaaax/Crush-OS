...
# with its streams redirected to a file (e.g. crush.ps1 run), Python defaults to the
...
from crush.analytics.registry import analytics_registry as _analytics_registry
from crush.bootstrap import build
from crush.capabilities.skills.dev_extensions import mount_dev_views
from crush.capabilities.skills.registry import skill_registry
from crush.engine.auth import verify_api_token  # ── [AUTH] ──
from crush.engine.background.routines import ROUTINES_ENABLED, Routine, RoutineStore
...
from crush.interfaces.api.deezer import router as deezer_router
...
from crush.interfaces.api.macropad_2k import _ui_router as macropad_ui_router
from crush.interfaces.api.macropad_2k import router as macropad_router
from crush.interfaces.api.music import router as music_router
from crush.interfaces.api.projects import router as projects_router
from crush.interfaces.api.routines import router as routines_router
from crush.interfaces.api.spotify import router as spotify_router
from crush.interfaces.api.voice_ws import router as voice_ws_router
from crush.interfaces.api.websocket import router as ws_router
from crush.interfaces.api.widgets import router as widgets_router
from crush.interfaces.channels.setup import setup_channels
from crush.interfaces.channels.telegram_bot import get_telegram_channel
from crush.kernel.paths import UI_STATIC_DIR
from crush.kernel.settings import settings
from crush.providers.audio.clap_detector import ClapDetector
from crush.providers.audio.tts import tts_engine
from crush.providers.memory.search import FTSIndex
from crush.providers.vision.daemon import run_vision_daemon
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    #  - `tracker` (crush.engine.tracking) reste module-level pour cette étape
    ...
    # Préchargement du TTS, en tâche de fond : le modèle Piper met ~2,6 s à
    # charger sur le Pi, et ce coût tomberait sinon sur la première réponse
...
# Auth en premier : /login doit répondre même si un autre routeur monte une
# route concurrente, et c'est la seule porte d'entrée non authentifiée.
...
from crush.interfaces.api.curator import router as curator_router  # noqa: E402

app.include_router(curator_router)

# ── [UI/M5] État de santé des connecteurs (Réglages → Connexions)
from crush.interfaces.api.connectors import router as connectors_router  # noqa: E402
...
def main() -> None:
    ...
    - `python -m crush.app` (entry point cible, B.4)
    ...
    uvicorn.run(
        ...
        reload_dirs=["src/crush", "prompts", "config"],
        log_level="warning",
    )
