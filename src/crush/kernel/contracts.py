...
class InitiativeStore(Protocol):
    """Lecture et arbitrage des initiatives proactives (cf. proactive/store.py).

    Déclaré ici pour que l'outil `initiatives` (L1) puisse les lire sans
    importer `engine` (L2), ce qu'interdit la RÈGLE 2 d'import-linter. Même
    motif que RemoteAgentRegistry.
    """
...
@runtime_checkable
class Tool(Protocol):
    """Interface des outils Crush (cf. tools/base.py)."""
...
class PushSortant(Protocol):
    ...
    # Une question, avec de quoi y repondre la ou elle arrive. Les canaux qui
    # ne savent pas afficher de choix recoivent le texte : ils perdent le
    # bouton, pas le message.
