...
from crush.kernel.vocab import AccessLevel, AutonomyLevel
...
# Anciennement : `from crush.providers.llm.api import ToolCapture` dans
...
PRICING: dict[str, dict[str, dict[str, float]]] = {
    ...
    # Les clés vont du plus spécifique au plus général : `calculate_cost`
    # retombe sur une correspondance par préfixe quand le modèle exact est
    # inconnu, et s'arrête à la première trouvée. « gemini-3-flash-preview »
    # doit donc précéder « gemini-3-flash ».
    "gemini": {
        "gemini-3-flash-preview": {"input_per_1m": 0.50, "output_per_1m": 3.00},
        "gemini-3-pro-preview": {"input_per_1m": 2.00, "output_per_1m": 12.00},
        "gemini-2.5-flash-lite": {"input_per_1m": 0.10, "output_per_1m": 0.40},
        # TTS : la sortie est de l'audio, facturé bien plus cher que du texte.
        # Doit précéder « gemini-2.5-flash », dont il porte le préfixe.
        "gemini-2.5-flash-preview-tts": {"input_per_1m": 0.50, "output_per_1m": 10.00},
        "gemini-2.5-pro-preview-tts": {"input_per_1m": 1.00, "output_per_1m": 20.00},
    ...
    "openai": {
        ...
        # STT — facturé à la minute d'audio, pas au token.
