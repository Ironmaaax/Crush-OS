...
"""Gateway de messagerie unifié Crush.
...
disque, et route chaque message entrant vers le core.Gateway Crush.
"""
...
from crush.engine.gateway import Gateway as CrushGateway
from crush.interfaces.channels.base import ChannelAdapter, IncomingMessage, MessageTarget
...
class MessagingGateway:
    ...
    Usage::
        ...
        gw = MessagingGateway(crush_gateway=app.state.gateway)
    ...
    def __init__(
        self,
        crush_gateway: CrushGateway,
        session_map_path: Path = _SESSION_MAP_FILE,
    ) -> None:
        self._crush = crush_gateway
    ...
    async def dispatch(self, msg: IncomingMessage) -> None:
        """Route un message entrant vers Crush et renvoie la réponse au bon canal.
        ...
        session, _route, response = await self._crush.handle(
            msg.text,
            session_id=session_id,
            stream=False,
        )
