...
// ═══════════════════════════════════════════════════════════════════════════
//  wake_sequence.js — Séquence de réveil Crush (étape A)
//
...
(function () {
    ...
    function runFacePhase(rootEl, opts, onDone) {
        ...
        function maybeShowVerdict() {
            ...
            if (recognized) {
                ...
                SFX.play('correct');
                // TTS — Crush dit "Identité vérifiée, bonjour {userName}"
    ...
    function createWakeSequence(rootEl, opts) {
        ...
        if (!S || !THREE || !window.createCrushOrb) {
            console.error('[wake] dépendances manquantes (SPHERE_STYLE / THREE / createCrushOrb)');
            return null;
        }
        ...
        // ── Montage orbe réel ──────────────────────────────────────────
        const orb = window.createCrushOrb(canvas, {
            frozen: true,
            onTick: onOrbTick,
        });
        if (!orb) {
    ...
    //  un CustomEvent 'crush-wake' sur window. Le sleep screen écoute ça.
    //
    ...
    function createSleepScreen(rootEl, opts) {
        ...
        rootEl.innerHTML = ''
            ...
            + '  <div class="wake-sleep-title">CRUSH</div>'
        ...
        window.addEventListener('crush-wake', handleWake);
        ...
        return {
            ...
            destroy: function () {
                window.removeEventListener('crush-wake', handleWake);
    ...
    if (!window.triggerWake) {
        window.triggerWake = function () {
            window.dispatchEvent(new CustomEvent('crush-wake', { detail: { trigger: 'manual' } }));
        };
    }
