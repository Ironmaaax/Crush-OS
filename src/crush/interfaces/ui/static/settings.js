...
(function () {
  ...
  async function saveSetting(key, value, btn) {
    ...
    try {
      ...
      if (resp.needs_restart) {
        J.notify({ kind: "error", text: key + " · redémarrage Crush requis" });
      } else {
  ...
  async function renderPreferences() {
    ...
    const fnInput = el("input", { class: "input-mono", style: { width:"220px" }, value: crush.user_firstname || "" });
    ...
    const anInput = el("input", { class: "input-mono", style: { width:"220px" }, value: crush.assistant_name || "" });
    ...
    const upInput = el("input", { class: "input-mono", style: { width:"220px" }, value: crush.user_profile || "", placeholder: "entrepreneur tech, Lyon…" });
    ...
    wrap.appendChild(ghostSec("Identité", "comment Crush s'adresse à toi", null, identList));

    // ── Langue & style ──
    const styleList = el("div");
    const qbTog = el("div", { class: "toggle" + (crush.quebec_mode ? " on" : "") });
    ...
    const wupTog = el("div", { class: "toggle" + (crush.wakeup_enabled ? " on" : "") });
    ...
    const clapTog = el("div", { class: "toggle" + (crush.clap_detection_enabled ? " on" : "") });
    ...
    wakeList.appendChild(settingRow("Détection de clap", "Double clap pour réveiller Crush", clapTog));
  ...
  async function renderModeles() {
    ...
    micSave.addEventListener("click", () => {
      if (window.CrushMic) {
        CrushMic.setSelectedDeviceId(micSelect.value);
    ...
    if (window.CrushMic) {
      CrushMic.listInputDevices().then((devices) => {
        devices.forEach((d) => {
          const opt = el("option", { value: d.deviceId, text: d.label });
          if (d.deviceId === CrushMic.getSelectedDeviceId()) opt.selected = true;
  ...
  async function renderConso() {
    ...
    // Le plafond vient de BUDGET_MONTHLY_USD, plus d'un 500 codé en dur qui
    // rendait la jauge « % consommé » purement décorative.
    const BUDGET = monthly.budget_usd || 10;
    ...
    // Extrapolation sur les jours RÉELLEMENT couverts par le registre, pas sur
    // le quantième du mois : un suivi démarré le 18 divisait sinon par 18 et
    // annonçait une fin de mois dix-huit fois trop basse.
    const daysInMonth = monthly.days_in_month
      || new Date(new Date().getFullYear(), new Date().getMonth() + 1, 0).getDate();
    const daysTracked = Math.max(1, monthly.days_tracked || 1);
    const forecast = monthlyCost / daysTracked * daysInMonth;
    ...
    // Aux montants en jeu ici, `toFixed(2)` écrase tout : trois postes à
    // $0.004 s'affichaient « $0.01 · $0.01 · $0.01 » et le centre du donut
    // annonçait « $0 » pour deux centimes. On garde assez de décimales pour
    // que le chiffre reste un chiffre.
    function fmtUSD(v) {
      const n = Number(v) || 0;
      if (n === 0) return "$0";
      const a = Math.abs(n);
    ...
    // Le même montant scindé en partie entière / décimales, pour les grandes
    // tuiles où la décimale est composée plus petite.
    function splitUSD(v) {
      const txt = fmtUSD(v).slice(1);
      const i = txt.indexOf(".");
      return i < 0 ? [txt, ""] : [txt.slice(0, i), txt.slice(i)];
    }
    ...
    // Les tarifs des fournisseurs sont publiés en dollars, Google facture en
    // euros. Sans les deux montants côte à côte, comparer le tableau de bord à
    // la facture demande une conversion mentale — et fait conclure à un écart
    // là où il n'y en a pas.
    const TAUX_EUR = monthly.usd_to_eur || 0.92;
    function fmtEUR(v) {
      const n = (Number(v) || 0) * TAUX_EUR;
      if (n === 0) return "0 €";
      const a = Math.abs(n);
      const dec = a < 0.01 ? 4 : a < 1 ? 3 : 2;
    ...
    const [whole1, dec1] = splitUSD(monthlyCost);
    v1.innerHTML = `$${whole1}<span class="dec">${dec1}</span>`
      + ` <span class="rel eur">≈ ${fmtEUR(monthlyCost)}</span>`;
    t1.appendChild(v1);
    const budgetTxt = `budget mensuel · $${BUDGET} · ${budgetPct.toFixed(budgetPct < 1 ? 1 : 0)}% consommé`
      + (monthly.budget_enabled === false ? " (alerte désactivée)" : "");
    t1.appendChild(el("div", { class: "d", text: budgetTxt }));
    ...
    const [whole2, dec2] = splitUSD(todayCost);
    v2.innerHTML = `$${whole2}<span class="dec">${dec2}</span>`;
    t2.appendChild(v2);
    ...
    const v4 = el("div", { class: "v gold" });
    v4.innerHTML = `${fmtUSD(forecast)}<span class="rel eur">≈ ${fmtEUR(forecast)}</span>`;
    t4.appendChild(v4);
    t4.appendChild(el("div", {
      class: "d sub",
      text: `extrapolé sur ${daysTracked} jour${daysTracked > 1 ? "s" : ""} de relevés`,
    }));
    ...
    /* ── Portée du relevé ──────────────────────────────────────────────────
     * Ce tableau ne connaît que ce que les providers lui ont poussé. Tout ce
     * qui a été dépensé avant la mise en place du suivi est invisible ici et
     * bien présent sur la facture du fournisseur. Sans cette ligne, l'écart
     * se lit comme une erreur de tarif. */
    const depuis = dateFR(monthly.tracked_since);
    const scope = el("div", { class: "conso-scope" });
    if (depuis) {
      const aujourdhui = dateFR(new Date().toISOString().slice(0, 10));
      scope.innerHTML =
        `<b>Relevé du ${depuis} au ${aujourdhui}</b> — ${daysTracked} jour`
        + `${daysTracked > 1 ? "s" : ""}. Ce total ne coïncidera pas avec la console`
        + ` Google, pour trois raisons distinctes :`
        + `<ul>`
        + `<li><b>Les appels d'avant le ${depuis} sont absents.</b> Le suivi n'existait`
        + ` pas encore ; ce qui a été dépensé avant ne sera jamais rattrapé.</li>`
        + `<li><b>Google publie avec jusqu'à 24 h de retard.</b> La dépense du jour`
        + ` figure ici et pas encore là-bas.</li>`
        + `<li><b>Unités différentes.</b> Ici, dollars hors taxes au tarif public ;`
        + ` Google facture en euros (conversion indicative à ${TAUX_EUR}).</li>`
        + `</ul>`
        + `Sur une période close et commune, les deux doivent se rejoindre.`;
    } else {
      scope.innerHTML =
        `<b>Aucun relevé pour l'instant.</b> Les compteurs se remplissent au premier`
        + ` échange. Les dépenses antérieures à la mise en place du suivi ne seront pas`
        + ` rattrapées.`;
    }
    ...
    li.appendChild(sw);
    li.appendChild(document.createTextNode(" Crush "));
    li.appendChild(el("span", { class: "val", text: fmtUSD(monthlyCost) }));
    ...
    if (byType.length) {
      byType.forEach(u => {
        ...
        vals.appendChild(el("div", { class: "val", text: fmtUSD(u.cost_usd) }));
    ...
    const PROV_COLORS = { anthropic: "#E5A23E", gemini: "#8AB4F8", elevenlabs: "#A78BFA", openai: "#36D399", deepgram: "#4A9EFF" };
    const donutSlices = provList.map(p => ({
      value: p.cost_usd || 0,
      color: PROV_COLORS[p.name] || "var(--fg-3)",
    }));
    const donutTotal = fmtUSD(monthlyCost);
    ...
    provList.forEach(p => {
      ...
      row.appendChild(el("span", { class: "val", text: fmtUSD(p.cost_usd) }));
    ...
    modTtl.appendChild(el("span", { class: "sub", text: "entrée / sortie · tarif appliqué" }));
    ...
    if (byModel.length) {
      byModel.slice(0, 5).forEach(m => {
        ...
        vals.appendChild(el("span", { class: "meta accent", text: fmtUSD(m.cost_usd) }));
        row.appendChild(vals);
        modCard.appendChild(row);

        // Le détail qui rend le total vérifiable à la main, et comparable aux
        // SKU de la console Google — qui séparent eux aussi les deux sens.
        const detail = el("div", { class: "conso-mod-detail" });
        const ti = m.input_tokens || 0, to = m.output_tokens || 0;
        const ri = m.rate_input_per_1m || 0, ro = m.rate_output_per_1m || 0;
        detail.appendChild(el("span", {
          text: `entrée ${ti.toLocaleString("fr-FR")} × $${ri.toFixed(2)}/M = ${fmtUSD(ti / 1e6 * ri)}`,
        }));
        detail.appendChild(el("span", {
          text: `sortie ${to.toLocaleString("fr-FR")} × $${ro.toFixed(2)}/M = ${fmtUSD(to / 1e6 * ro)}`,
        }));
        detail.appendChild(el("span", { text: `${m.calls || 0} appels` }));
        if (!ri && !ro) {
          // Tarif absent de la table : le coût affiché vaut zéro et le total
          // est donc faux. Le dire, plutôt que de laisser croire au gratuit.
          detail.appendChild(el("span", { class: "alerte", text: "⚠ tarif inconnu — non facturé ici" }));
        }
        modCard.appendChild(detail);
      });
    } else {
    ...
    /* Derniers appels — remplace un encart « Par skill · bientôt disponible »
     * qui n'a jamais rien affiché. C'est le seul endroit où l'on voit qu'une
     * question banale déclenche cinq appels, dont trois pour la mémoire : les
     * agrégats du dessus donnent le total, pas le mécanisme. */
    const callsCard = el("div", { class: "card" });
    const callsHd = el("div", { class: "card-hd" });
    const callsTtl = el("div");
    callsTtl.appendChild(el("h2", { text: "Derniers appels" }));
    callsTtl.appendChild(el("span", { class: "sub", text: "aujourd'hui · du plus récent" }));
    callsHd.appendChild(callsTtl); callsCard.appendChild(callsHd);

    const CTX_LABELS = {
      conversation: "échange", memory: "mémoire", proactive: "proactif",
      voice: "voix", mission: "mission", skill: "skill",
    };
    const recent = (session.total_api_calls ? await J.api.get("/api/conso/calls").catch(() => []) : []);
    if (recent.length) {
      recent.slice(0, 12).forEach(c => {
        const row = el("div", { class: "conso-mod-row" });
        const ctx = (c.context || "").split(":")[0];
        const heure = (c.timestamp || "").slice(11, 19);
        row.appendChild(el("div", { class: "nm", text: `${heure}  ${CTX_LABELS[ctx] || ctx || "—"}` }));
        const vals = el("div", { class: "conso-mod-vals" });
        const io = c.audio_minutes
          ? `${c.audio_minutes.toFixed(2)} min`
          : `${fmtTokens(c.input_tokens || 0)} → ${fmtTokens(c.output_tokens || 0)}`;
        vals.appendChild(el("span", { class: "meta", text: io }));
        vals.appendChild(el("span", { class: "meta accent", text: fmtUSD(c.cost_usd) }));
        row.appendChild(vals);
        callsCard.appendChild(row);
      });
    } else {
      callsCard.appendChild(el("div", { class: "conso-empty", text: "— aucun appel aujourd'hui" }));
    }
    row3.appendChild(callsCard);
    wrap.appendChild(row3);

    const page = pageWrapper("conso", "Ce que Crush consomme pour fonctionner", null, wrap);
    root.innerHTML = ""; root.appendChild(page);
  }
  ...
  async function renderSysteme() {
    ...
    if (Object.keys(stats).length) {
      ...
      wrap.appendChild(ghostSec("Statistiques Crush", "projets · mémoire · sessions", null, statGrid));
    }
    ...
    restartTxt.appendChild(el("div", { class: "danger-label", text: "Redémarrer Crush" }));
    ...
    restartBtn.addEventListener("click", async () => {
      if (!confirm("Redémarrer Crush ?")) return;
  ...
  async function renderApropos() {
    ...
    nameWrap.appendChild(el("div", { class: "about2-name", text: window.CRUSH_ASSISTANT_NAME || "Crush" }));
    ...
    const srcLink = el("a", {
      class: "about2-src",
      href: "https://github.com/Ironmaaax/Crush-OS",
    ...
    updateBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        if (res.already_up_to_date) {
          updateStatus.textContent = "Crush est déjà à jour.";
          updateStatus.className = "about2-update-status ok";
          updateBtn.textContent = "À jour";
        } else if (res.ok) {
          updateStatus.textContent = "Mise à jour réussie. Redémarre Crush pour appliquer les changements.";
          updateStatus.className = "about2-update-status ok";
    ...
    const page = pageWrapper("apropos", "À propos de Crush", null, wrap);
    root.innerHTML = ""; root.appendChild(page);
  }
