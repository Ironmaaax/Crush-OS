/* globe.js — Crush Globe v4 — Mapbox GL JS */
(function () {
  'use strict';
  ...
  async function _init() {
    ...
    try {
      ...
      if (r.ok) { const cfg = await r.json(); token = cfg.mapbox_token || ''; motif = cfg.mapbox_motif || ''; }
    } catch { }
    if (!token) {
      // Un console.warn laissait un conteneur vide : l'utilisateur voyait
      // un globe qui ne s'affiche pas, sans la moindre indication. On le
      // dit dans la page, avec le remede.
      console.warn('[Globe] MAPBOX_TOKEN absent');
      const hote = document.getElementById('globe-container');
      if (hote) {
        hote.innerHTML =
          '<div style="display:flex;align-items:center;justify-content:center;'
          + 'height:100%;padding:24px;"><div style="max-width:46ch;text-align:center;'
          + 'line-height:1.6;color:var(--fg-1,#e8e8e8);font-size:13.5px;">'
          + (motif || 'Vue globe indisponible : aucun jeton Mapbox. Créer un jeton '
            + 'public gratuit sur account.mapbox.com, puis ajouter MAPBOX_TOKEN=… '
            + 'dans le .env du serveur et redémarrer crush-api.')
          + '</div></div>';
      }
      return;
    }
  ...
  // ── Crush.views registration ────────────────────────────────────
  if (window.Crush?.views) {
    Crush.views.register('globe', {
      meta: {
        name: 'Globe',
