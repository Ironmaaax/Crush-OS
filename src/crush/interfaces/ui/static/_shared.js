/* ─────────────────────────────────────────────────────────────────────
   CRUSH — Shared runtime v2 (vanilla JS, ES2017+)
   Public API attached to window.Crush :

     Crush.mountAtmosphere()
     Crush.setMode(mode)                        → "home"|"workspace"|"capacites"|"config"
     Crush.mountRooms({ mode, pages, activePage, onNav, onMissionControl })
     Crush.mountTopbar({ pageTitle, crumb, onAsk, onCmdK })
     Crush.openCmdK() / .closeCmdK()
     Crush.openMissionControl() / .closeMissionControl()
     Crush.registerCommands(arr)
     Crush.notify({ kind, text })
     Crush.api.get(path) / .post(path, body)
     Crush.fmt.num(v) / .pct(v) / .relTime(d)
     Crush.sparkline(data, opts)
...
(function () {
  ...
  const Crush = (window.Crush = window.Crush || {});
  ...
  Crush.currentTheme = function () {
    try {
      const id = localStorage.getItem("crush_theme");
      return id && THEMES[id] ? id : _DEFAULT_THEME;
    } catch (e) { return _DEFAULT_THEME; }
  };

  Crush.applyTheme = function (id) {
    ...
    try { localStorage.setItem("crush_theme", THEMES[id] ? id : _DEFAULT_THEME); } catch (e) {}
    window.dispatchEvent(new CustomEvent("crush:theme", { detail: { id: id, rgb: t.rgb } }));
  };

  // Applique le thème sauvegardé tout de suite (avant le paint → pas de flash).
  Crush.applyTheme(Crush.currentTheme());
  ...
  function authHeaders(extra) {
    ...
    const token = window.CRUSH_API_TOKEN;
    if (token) headers.Authorization = "Bearer " + token;
    return headers;
  }

  Crush.api = {
    base: window.CRUSH_API_BASE || "",
    async get(path) {
  ...
  Crush.authHeaders = authHeaders;

  /* ───────── Navigation (iframe-aware) ───────── */
  Crush.navigate = function (url) {
    // Si on tourne dans l'iframe, déléguer au shell parent
    if (window !== window.top && window.top.Crush?.navigateFrame) {
      window.top.Crush.navigateFrame(url);
      return;
    }
    // Si le shell home a enregistré un handler iframe
    if (typeof Crush.navigateFrame === "function") {
      Crush.navigateFrame(url);
      return;
    }
  ...
  /* ───────── Atmosphere ───────── */
  Crush.mountAtmosphere = function () {
  ...
  /* ───────── Mode system ───────── */
  const MODE_META = {
    home:       { chapter: "—",  num: "00", label: window.CRUSH_ASSISTANT_NAME || "CRUSH",        watermark: "" },
  ...
  /* ───────── Scene card ───────── */
  Crush.showSceneCard = function (chapter, title, duration) {
  ...
  Crush.mountRooms = function (opts) {
    _roomsOpts = opts;
    ...
    if (!mc) {
      mc = el("button", { id: "j-rooms-mc", class: "rooms-mc", onclick: () => Crush.openMissionControl() });
  ...
  function ensureMissionControl() {
    if (mcRoot) return;
    mcRoot = el("div", { class: "mission-overlay is-hidden", onclick: (e) => { if (e.target === mcRoot) Crush.closeMissionControl(); } });
    ...
    header.appendChild(el("button", { class: "mc-close", onclick: () => Crush.closeMissionControl(), text: "×" }));
    ...
    ROOMS.forEach(r => {
      const card = el("button", {
        class: "mission-card",
        dataset: { mode: r.mode },
        onclick: () => { Crush.closeMissionControl(); Crush.navigate(r.href); },
      });
  ...
  Crush.openMissionControl = function () {
    ensureMissionControl();
    mcOpen = true;
    mcRoot.classList.remove("is-hidden");
  };
  Crush.closeMissionControl = function () {
    if (!mcRoot) return;
    mcOpen = false;
  ...
  */
  Crush.mountSidebar = function (opts) {
    ...
    root.appendChild(el("div", { class: "sb-brand" }, [
      ...
      el("div", { class: "sb-brand-text" }, [
        el("span", { class: "sb-brand-name", text: window.CRUSH_ASSISTANT_NAME || "Crush" }),
  ...
  /* ───────── Topbar ───────── */
  Crush.mountTopbar = function (opts) {
  ...
  const _navCmds = ROOMS.map(r => ({
    ...
    run: () => { Crush.navigate(r.href); },
  }));
  cmdkCommands = _navCmds;

  Crush.registerCommands = function (cmds) {
  ...
  function ensureCmdK() {
    if (cmdkRoot) return;
    cmdkRoot = el("div", { class: "cmdk-back is-hidden", onclick: (e) => { if (e.target === cmdkRoot) Crush.closeCmdK(); } });
    ...
    cmdkInput = el("input", {
      ...
      onkeydown: (e) => {
        if (e.key === "Escape")    { e.preventDefault(); Crush.closeCmdK(); }
  ...
  function currentResults() {
    ...
    if (v.startsWith(">")) {
      ...
      if (q.startsWith("ask ")) {
        const p = q.slice(4);
        return [{ kind: "ask", id: "ask", title: 'Demander : "' + p + '"', glyph: "›", group: "Faire", run: () => _askCrush(p) }];
      }
  ...
  async function _askCrush(prompt) {
    Crush.notify({ kind: "info", text: "Crush · réflexion…" });
    try {
      const r = await Crush.api.post("/api/agent/ask", { prompt });
      Crush.notify({ kind: "success", text: (r.answer || r.text || "").slice(0, 240) });
    } catch (err) {
      Crush.notify({ kind: "error", text: "Échec : " + err.message });
    }
  }
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if ((e.metaKey || e.ctrlKey) && (e.key === "k" || e.key === "K")) {
      e.preventDefault();
      cmdkOpen ? Crush.closeCmdK() : Crush.openCmdK();
    }
    // ⌘T — Mission Control
    if ((e.metaKey || e.ctrlKey) && (e.key === "t" || e.key === "T")) {
      e.preventDefault();
      mcOpen ? Crush.closeMissionControl() : Crush.openMissionControl();
    }
    // Escape
    if (e.key === "Escape") {
      if (cmdkOpen) Crush.closeCmdK();
      if (mcOpen)   Crush.closeMissionControl();
    }
  });
  ...
  /* ───────── Notification ribbon ───────── */
  let notifStack = null;
  Crush.notify = function ({ kind = "info", text = "" }) {
    if (!notifStack) {
  ...
  /* ───────── Sparkline ───────── */
  Crush.sparkline = function (data, opts) {
    opts = opts || {};
  ...
  Crush.mountBottomNav = function (opts) {
