...
(function () {
  ...
  const CONFIGS = {
    "fusion360": {
      fullDesc: "Contrôle Autodesk Fusion 360 via MCP HTTP — scripts Python API, lecture et modification de designs 3D, undo/redo, export et lancement d'actions Fusion directement depuis Crush.",
      fields: [
    ...
    "mode-streameur": {
      isRoutine: true,
      fullDesc: "Lance l'environnement stream complet — OBS Studio, scènes préconfigurées, overlay Twitch. Crush orchestre le démarrage, gère les transitions et t'alerte si une app ne répond pas.",
  ...
  function buildViewPanel(panel, v) {
    ...
    // Statut actif
    const registered = window.top?.Crush?.views?.list() || [];
  ...
  // il fallait ouvrir une session SSH et lire des journaux. Sur une machine sans
  // écran, consultée depuis un téléphone, c'est inaccessible — et c'est
  // justement quand ça cloche qu'on ne peut pas se le permettre.
  //
  // Chaque maillon montre son état ET son remède. Un voyant rouge sans remède
  // ne vaut guère mieux qu'un silence.
  const ECO_FAMILLES = [
    ["cerveau",      "Cerveau",      "le modèle qui répond"],
    ["voix",         "Voix",         "entendre et parler"],
    ["memoire",      "Mémoire",      "ce qu'il retient de toi"],
    ["autonomie",    "Autonomie",    "ce qu'il fait sans qu'on le demande"],
    ["garde-fous",   "Garde-fous",   "ce qui l'empêche de nuire ou de trop coûter"],
    ["integrations", "Services",     "ce à quoi il est relié"],
  ];

  const ECO_ETATS = {
    ok:      { libelle: "opérationnel", couleur: "green" },
    degrade: { libelle: "dégradé",      couleur: "accent" },
    absent:  { libelle: "non configuré", couleur: "muted" },
  };

  async function renderEcosysteme() {
    let data = null;
    try { data = await J.api.get("/api/ecosysteme"); } catch (_) {}

    const wrap = el("div", { style: { display: "flex", flexDirection: "column", gap: "28px" } });

    if (!data || !Array.isArray(data.maillons)) {
      wrap.appendChild(el("div", { class: "j-empty", text: "État indisponible — le serveur n'a pas répondu." }));
      const page = pageWrapper("ecosysteme", "L'écosystème Crush", null, wrap);
      root.innerHTML = ""; root.appendChild(page);
      return;
    }

    const r = data.resume || {};
    // Le sous-titre porte le seul chiffre qui décide d'agir ou non : ce qui ne
    // va pas. Compter les éléments sains ferait un tableau de bord flatteur et
    // inutile.
    const aTraiter = (r.degrade || 0) + (r.absent || 0);
    const soustitre = aTraiter === 0
      ? '<span class="v">tout</span> est en place'
      : '<span class="v">' + aTraiter + '</span> point' + (aTraiter > 1 ? "s" : "") + " à regarder";

    ECO_FAMILLES.forEach(([cle, titre, sous]) => {
      const membres = data.maillons.filter(m => m.famille === cle);
      if (!membres.length) return;

      const liste = el("div", { class: "eco-liste" });
      membres.forEach(m => {
        const etat = ECO_ETATS[m.etat] || ECO_ETATS.absent;
        const ligne = el("div", { class: "eco-ligne eco-ligne--" + etat.couleur });

        const gauche = el("div", { class: "eco-gauche" });
        gauche.appendChild(el("div", { class: "eco-nom", text: m.nom }));
        gauche.appendChild(el("div", { class: "eco-detail", text: m.detail || "" }));
        // Le remède n'apparaît que s'il y en a un à appliquer : afficher
        // « rien à faire » sous chaque ligne saine noierait les trois qui
        // demandent une action.
        if (m.remede) gauche.appendChild(el("div", { class: "eco-remede", text: m.remede }));
        ligne.appendChild(gauche);

        ligne.appendChild(el("div", { class: "dv-badge " + etat.couleur, text: etat.libelle }));
        liste.appendChild(ligne);
      });

      wrap.appendChild(ghostSec(titre, sous, null, liste));
    });

    const page = pageWrapper("ecosysteme", "L'écosystème Crush", soustitre, wrap);
    root.innerHTML = ""; root.appendChild(page);
  }
  ...
  async function buildMacropadPanel(panel) {
    ...
    body.appendChild(el("div", { class: "panel-desc",
      text: "Macropad 2 touches sur puce CH552. Configure les raccourcis, l'éclairage et flash le firmware directement depuis Crush." }));
          ...
          try {
            ...
            if (!msgs.length) {
              ...
              msgs.forEach(m => {
                ...
                const label  = el("div", { class: "sess-bubble-role", text: m.role === "user" ? "Toi" : (window.CRUSH_ASSISTANT_NAME || "Crush") });
