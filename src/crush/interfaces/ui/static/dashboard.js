...
(function () {
  ...
  async function loadInitiatives() {
    try {
      ...
      return raw.map(i => ({
        ...
        source:   i.context ? i.context.slice(0,40) : (window.CRUSH_ASSISTANT_NAME || "Crush"),
  ...
  async function renderInitiatives() {
    ...
    wrap.appendChild(ghostSec(
      "En attente",
      inits.length + " initiatives · proactif " + (window.CRUSH_ASSISTANT_NAME || "Crush"),
      null,
      list
    ));
  ...
  function buildInitPanel(panel, i) {
    ...
    info.appendChild(el("div", { class: "mp-sub", text: i.source || ((window.CRUSH_ASSISTANT_NAME || "Crush") + " proactif · " + (i.due || "")) }));
  ...
  function renderInitRow(i, showActions) {
    ...
    meta.appendChild(el("span", { text: i.source || (window.CRUSH_ASSISTANT_NAME || "Crush") }));
    meta.appendChild(el("span", { style: { opacity: ".4" }, text: "·" }));
  ...
  async function renderAnalytics() {
    ...
    // Crush KPIs from /api/analytics/crush
    let crushData = null;
    try { crushData = await J.api.get("/api/analytics/crush?days=30"); } catch (_) {}

    if (crushData) {
      const kpiGrid = el("div", { class: "kpi-strip" });
      const kpis = [
        { lbl: "Requêtes 30j", val: crushData.total_calls||0, unit: "" },
        { lbl: "Coût 30j",     val: crushData.total_cost ? "$"+crushData.total_cost.toFixed(2) : "—", unit: "" },
        { lbl: "Tokens",       val: crushData.total_tokens||0, unit: "" },
      ];
      kpis.forEach(k => {
