(function () {
  ...
  const form = {
    user_firstname: "",
    assistant_name: window.CRUSH_ASSISTANT_NAME || "Crush",
  ...
  function renderWelcome(status) {
    ...
    if (py.runtime_source) {
      pyLines.push("Runtime Crush · " + py.runtime_source
  ...
  function renderIdentity() {
    ...
    card.appendChild(field("Nom de l'assistant", "assistant_name", { placeholder: window.CRUSH_ASSISTANT_NAME || "Crush" }));
  ...
  function renderFinish() {
    ...
    card.appendChild(el("p", {
      text: "Crush est prêt. Lance le serveur principal depuis le terminal, puis ouvre l'interface.",
    }));
    if (doneUrl) {
    ...
    card.appendChild(el("p", {
      ...
      text: "Windows : .\\crush.ps1 run     ·     Linux/macOS : crush run",
    }));
    card.appendChild(el("p", {
      class: "setup-lead",
