...
function resetInactivityTimer() {
    ...
    if (window._crushSleeping) return;
...
async function enterSleepMode() {
    ...
    gsap.to(['#crush-toggle', '#globe-toggle', '#intel-toggle',
...
async function showScanResult(result) {
    ...
    if (result.recognized) {
        ...
        name.textContent = (_userFirstname || result.name || (window.CRUSH_ASSISTANT_NAME || 'CRUSH')).toUpperCase();
...
// ═══════════════════════════════════════════════════════════════════════════
//  PHASE 5 — Déploiement UI + voix Crush
// ═══════════════════════════════════════════════════════════════════════════
...
async function deployUI() {
    ...
    tl.to(['#crush-toggle', '#globe-toggle', '#intel-toggle',
            '#dashboard-nav-btn', '#settings-nav-btn', '#panels-toggle'],
    ...
    // Voix Crush (après 0.8s pour que l'UI soit visible)
    await _delay(800);
    try {
...
async function triggerWakeUp() {
    ...
    try {
        ...
        if (granted) {
            // Phase 5 — Déployer UI
            await deployUI();
            console.log('[WakeUp] Accès accordé — Crush actif');
        } else {
