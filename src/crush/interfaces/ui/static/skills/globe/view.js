/**
 * Vue Globe — crush-skills
 * Enregistre la vue "globe" dans Crush.views.
 * Dépend de : _shared.js (Crush.views), Mapbox GL JS (chargé dynamiquement)
 */
(function () {
  if (!window.Crush?.views) return;
  ...
  async function fetchToken() {
    ...
    // Le motif du refus vient du serveur : lui seul sait s'il manque un
    // jeton ou si le plafond mensuel est atteint. Les deux laissent la
    // carte noire mais appellent des gestes opposes.
    _motifRefus = data.mapbox_motif || '';
  ...
  Crush.views.register(VIEW_ID, {
    ...
    // Bindings gestuels lus par le routeur crush-OS quand la vue a le focus.
    // Clé = geste standard (vocabulaire crush-OS), valeur = commande ou action.
    gestures: {
      ...
      // pinch_y volontairement non bindé ici → retombe sur le volume global (crush-OS)
    },
    ...
    async show(params = {}) {
      ...
      try {
        ...
        // Verifier AVANT de charger la bibliotheque : construire une Map sans
        // jeton fait lever a mapbox-gl un « An API access token is required to
        // use Mapbox GL », message exact mais inexploitable — il ne dit ni
        // quel reglage manque, ni ou l'obtenir. L'autre implementation
        // (static/globe.js) avait ce garde-fou, pas celle-ci.
        if (!token) {
          throw new Error(
            _motifRefus ||
            "Vue globe indisponible : aucun jeton Mapbox. " +
            "Creer un jeton public gratuit sur account.mapbox.com, puis ajouter " +
            "MAPBOX_TOKEN=... dans le fichier .env du serveur et redemarrer " +
            "crush-api."
          );
        }
        await loadMapbox();
        ...
        // Un remede de deux phrases doit rester lisible : on aere et on borne
        // la largeur, sinon le message s'etale sur toute la vue en une ligne.
        container.innerHTML =
          ...
          'color:var(--fg-1,#e8e8e8);font-family:system-ui,sans-serif;font-size:13.5px;">' +
