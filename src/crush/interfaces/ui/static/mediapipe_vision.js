...
if (window.Crush?.gestures) {
  Crush.gestures.registerGlobal({
...
function _sendEvent(payload) {
  const sid = window._crushSessionId?.();
  window._crushWsSend?.({ type: 'vision_event', session_id: sid, ...payload });
}
