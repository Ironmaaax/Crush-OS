...
"""
Crush Voice Agent — LiveKit Agents pipeline vocal.
...
from crush.kernel.paths import PROJECT_ROOT  # noqa: E402
from crush.kernel.settings import settings
from crush.providers.audio.elevenlabs_voices import resolve_voice_id
...
logger = logging.getLogger("crush-voice")
...
def _make_livekit_tool(crush_tool: object) -> lk_llm.RawFunctionTool:
    """Wraps un Crush Tool comme LiveKit RawFunctionTool."""
    schema = crush_tool.to_claude_schema()  # type: ignore[attr-defined]
    raw_schema = {
        "name": schema["name"],
    ...
    async def _execute(raw_arguments: dict[str, object]) -> str:
        result = await crush_tool.execute(**raw_arguments)  # type: ignore[attr-defined]
...
def _build_voice_tools() -> list:
    """Retourne les LiveKit tools en miroir du mode texte (crush.app).
    ...
    de MemoryKernel). Les 13 `__import__("crush.capabilities.tools.X")`
    ...
    crush_tools = list(container.tool_registry._tools.values())
    ...
    crush_tools = [
        ...
        for t in crush_tools
    ]

    tools = [_make_livekit_tool(t) for t in crush_tools]
...
def prewarm(proc: object) -> None:
    ...
    logger.info("=" * 40)
    logger.info("✓ Crush vocal prêt — clique sur le micro")
    logger.info("=" * 40)
...
def _build_voice_stt(env: dict) -> object:
    ...
    # but Crush never receives a transcript (mic active, no answer to voice). Surface
    ...
    if not dg_key or any(c.isspace() for c in dg_key) or len(dg_key) < 20:
        logger.error(
            ...
            "ne fonctionnera pas : le micro capte mais Crush ne comprend rien. "
...
async def entrypoint(ctx: object) -> None:
    ...
    agent = CrushVoiceAgent(instructions=instructions, tools=tools)
...
def main() -> None:
    ...
    - `python -m crush.interfaces.voice.agent <args>` (entry point cible)
