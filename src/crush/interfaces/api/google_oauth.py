...
# In-memory state store (single-user CRUSH)
...
def _maybe_write_credentials_from_env(request: Request) -> None:
    ...
    # 0o600 : ce fichier porte le client_secret. L'ecriture initiale se
    # faisait en 644, alors que le rafraichissement en aval, lui, restreignait
    # bien ses droits — la protection ne valait donc que pour les jetons deja
    # renouveles au moins une fois.
    ecrire_atomique(path, json.dumps(config, indent=2), mode=0o600)
...
async def google_callback(
    ...
    try:
        ...
        # Contient le refresh_token, qui vaut un mot de passe et ne perime pas.
        ecrire_atomique(token_path, flow.credentials.to_json(), mode=0o600)
