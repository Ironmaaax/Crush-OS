...
from crush.kernel.approvals import ApprovalMode, approval_config, save_approval_config
from crush.kernel.permissions import PERMISSION_KEYS
from crush.kernel.permissions import permissions as _perm_store
...
async def get_permissions() -> dict[str, bool]:
    """Retourne l'état courant des permissions runtime (relu du disque)."""
...
async def patch_permission(key: str, body: PermissionPatch) -> dict[str, object]:
    ...
    L'écriture passe par le store persistant : le réglage survit au
    redémarrage du service.
    """
    if not _perm_store.set(key, body.enabled):
        # Auparavant le store ignorait la clé en silence et l'API répondait
        # 200 : l'utilisateur croyait avoir autorisé quelque chose.
        connues = ", ".join(sorted(PERMISSION_KEYS))
        raise HTTPException(404, f"Permission inconnue : '{key}'. Clés acceptées : {connues}.")

    reponse: dict[str, object] = {"key": key, "enabled": _perm_store.get(key), "persisted": True}
    if _perm_store.storage_error is not None:
        reponse["persisted"] = False
        reponse["warning"] = _perm_store.storage_error
    return reponse
