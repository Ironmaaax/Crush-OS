...
from crush.kernel.quota_cartes import consommer
...
async def get_config() -> dict[str, Any]:
    ...
    jeton = settings.mapbox_token.get_secret_value() or ""
    motif = ""

    if jeton:
        # Un chargement est compté à la REMISE du jeton, parce que c'est le
        # seul moment que le serveur maîtrise. Sans jeton, aucune carte ne se
        # construit : refuser ici suffit à tenir le plafond, là où un compteur
        # côté navigateur se contournerait d'un rechargement et ignorerait les
        # autres appareils.
        quota = consommer(settings.mapbox_monthly_limit)
        if quota.depasse:
            jeton = ""
            motif = (
                f"Plafond mensuel atteint : {quota.chargements} chargements de carte "
                f"ce mois-ci, limite fixée à {quota.plafond}. La carte se réactivera "
                f"le 1er du mois prochain. Pour relever la limite : "
                f"MAPBOX_MONTHLY_LIMIT dans le .env — Mapbox facture au-delà de "
                f"50 000 chargements par mois."
            )
    else:
        motif = (
            "Aucun jeton Mapbox. Créer un jeton public gratuit sur "
            "account.mapbox.com, puis ajouter MAPBOX_TOKEN=… dans le .env du "
            "serveur et redémarrer crush-api."
        )
    ...
    return {
        ...
        "mapbox_token": jeton,
        # Distinguer « pas configuré » de « plafond atteint » : les deux
        # laissent la carte noire, mais appellent des gestes opposés.
        "mapbox_motif": motif,
    }
