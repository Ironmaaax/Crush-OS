...
from crush.engine.mission.project_store import WORKSPACE_DIR
...
async def crush_doctor() -> dict:
    """Rapport de santé complet de tous les composants Crush."""
    import asyncio
...
async def system_stats(request: Request) -> dict:
    ...
    proj_files = list(WORKSPACE_DIR.glob("*/.crush/state.json")) if WORKSPACE_DIR.exists() else []
...
async def system_perf() -> dict:
    """Métriques temps réel : CPU, RAM, disque, batterie, process Crush."""
    import os as _os
    import platform
    import time
...
async def cleanup_done_projects(request: Request) -> dict:  # noqa: ARG001
    ...
    removed = 0
    for state_file in list(WORKSPACE_DIR.glob("*/.crush/state.json")):
        try:
...
async def conso_monthly(request: Request) -> dict:
    """Totaux du mois, augmentés du plafond réellement configuré.

    L'interface affichait un budget de 500 $ codé en dur, sans rapport avec
    `BUDGET_MONTHLY_USD` : la jauge « % consommé » ne voulait donc rien dire.
    """
    totaux = request.app.state.container.tracker.get_monthly_totals()
    totaux["budget_usd"] = settings.budget_monthly_usd
    totaux["budget_enabled"] = settings.budget_enabled
    # Google facture en euros : sans conversion, les deux chiffres ne se
    # comparent pas, même quand ils portent sur la même période.
    totaux["usd_to_eur"] = settings.usd_to_eur
    return totaux
