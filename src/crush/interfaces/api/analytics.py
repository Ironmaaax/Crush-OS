...
"""Analytics — stats Crush locales + YouTube Data API v3 + routes API.
...
from crush.analytics.registry import analytics_registry
...
async def get_crush_stats(days: int = 30) -> dict:
    """Stats d'usage Crush depuis les fichiers locaux."""
...
async def get_analytics_summary() -> dict:
    ...
    crush_task = asyncio.create_task(get_crush_stats(30))
    yt_task = asyncio.create_task(get_youtube_stats(7))
    crush, youtube = await asyncio.gather(crush_task, yt_task, return_exceptions=True)
    return {
        "crush": crush if not isinstance(crush, Exception) else None,
...
@router.get("/api/analytics/crush")
async def analytics_crush(days: int = 30) -> dict:
    return await get_crush_stats(days)
