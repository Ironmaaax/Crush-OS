...
"""Exécuteur de presets Crush — gère l'exécution step par step."""
...
from crush.capabilities.skills.app_checker import check_all_apps
from crush.capabilities.skills.base import PresetSkill, PresetStep
from crush.kernel.notifications import broadcast_audio
...
class PresetExecutor:
    ...
    async def _exec_ai(self, step: PresetStep) -> dict:
        ...
        # `max_tokens` n'existe pas sur LLMProvider.complete() : le passer
        # levait un TypeError et faisait échouer toute étape `ai`. La brièveté
        # est déjà demandée par la consigne ci-dessous.
