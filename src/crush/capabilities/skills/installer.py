...
"""Installation/désinstallation des skills depuis le dépôt de catalogue."""
...
from crush.capabilities.skills.registry import SKILLS_INSTALLED_DIR, skill_registry
from crush.kernel.paths import UI_STATIC_DIR
from crush.kernel.settings import settings

ENV_FILE = Path(".env")

# Le depot du catalogue est un reglage, pas une constante : la boutique
# telecharge du code depuis cette URL, donc en changer le nom sans que le depot
# existe ne renomme rien — ca casse l'installation. Voir settings.skills_catalog_repo.
def _base_depot() -> str:
    return f"https://raw.githubusercontent.com/{settings.skills_catalog_repo}/main"


def _url_index() -> str:
    return f"{_base_depot()}/index.json"
...
class SkillInstaller:
    ...
    async def _install_skill(
        ...
        async with httpx.AsyncClient(timeout=15) as client:
            r = await client.get(f"{_base_depot()}/{path}/skill.py")
            if r.status_code != 200:
                raise Exception(
                    f"skill.py introuvable dans {settings.skills_catalog_repo} "
                    f"(HTTP {r.status_code}) — vérifier SKILLS_CATALOG_REPO"
                )
            (skill_dir / "skill.py").write_text(r.text)

            r = await client.get(f"{_base_depot()}/{path}/skill.yaml")
            if r.status_code == 200:
            ...
            if static_files:
                ...
                async with httpx.AsyncClient(timeout=15) as client:
                    for fname in static_files:
                        r = await client.get(f"{_base_depot()}/{path}/static/{fname}")
                        if r.status_code == 200:
    ...
    async def _install_view(
        ...
        async with httpx.AsyncClient(timeout=15) as client:
            ...
            for fname in targets:
                r = await client.get(f"{_base_depot()}/{path}/{fname}")
                if r.status_code == 200:
                    (static_dst / fname).write_bytes(r.content)
