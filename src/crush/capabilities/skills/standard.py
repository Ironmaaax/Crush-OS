...
class AgentSkillsAdapter:
    """Convertit entre le format Crush (skill.yaml + skill.py) et agentskills.io (SKILL.md).
    ...
    Exemples::
        ...
        # Exporter un skill Crush vers le standard
        skill_md = AgentSkillsAdapter.export_to_standard("web-research")
    """

    # ── Import : SKILL.md → Crush ────────────────────────────────────────────
    ...
    def import_from_standard(cls, source: str | Path) -> str:
        ...
        (dest / "skill.yaml").write_text(
            yaml.dump(
                cls._standard_to_crush_yaml(fm, body),
                Dumper=_BlockDumper,
                allow_unicode=True,
                sort_keys=False,
            ),
    ...
    def export_to_standard(cls, skill_name: str) -> str:
        """Exporte un skill Crush au format SKILL.md agentskills.io.
        ...
        with skill_yaml_path.open(encoding="utf-8") as f:
            crush_meta: dict = yaml.safe_load(f) or {}

        return cls._crush_yaml_to_skill_md(crush_meta)
    ...
    @staticmethod
    def _standard_to_crush_yaml(fm: dict, body: str) -> dict:
        """Convertit frontmatter agentskills.io → skill.yaml Crush."""
    ...
    @staticmethod
    def _crush_yaml_to_skill_md(meta: dict) -> str:
        """Génère un SKILL.md agentskills.io depuis un skill.yaml Crush."""
    ...
    def _ensure_standard_frontmatter(
        ...
        return cls._crush_yaml_to_skill_md(meta)
...
def _generate_skill_py(skill_name: str) -> str:
    """Génère un skill.py Crush minimaliste depuis le nom du skill."""
