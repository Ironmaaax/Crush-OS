...
"""Alias de namespace `skills` → `crush.capabilities.skills` — CDC §B.2bis.
...
contiennent du code utilisateur sur disque qui importe :
    ...
    from crush.capabilities.skills.base import SkillBase, PresetSkill
...
la migration `skills/` → `crush/capabilities/skills/`.

Solution retenue (CDC) : enregistrer `skills` comme alias de
`crush.capabilities.skills` dans `sys.modules` AVANT tout chargement de
...
from crush.capabilities.skills import base as _base  # noqa: E402
