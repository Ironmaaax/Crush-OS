...
"""
Outil Fusion 360 pour Crush — Autodesk MCP HTTP (port 27182 par défaut).
...
Cet outil n'est PAS dans le registre global, et ne doit pas y entrer.
Son unique appelant est `engine/mission/worker_agent.py`, qui l'importe
paresseusement quand une étape de mission nomme `fusion_360` (dérogation
import-linter assumée, cf. pyproject.toml). L'ajouter au registre le
placerait dans la liste d'outils envoyée à CHAQUE requête — 4,3 Ko de
schéma, le plus gros du dépôt — pour piloter un logiciel qui n'existe ni
sur ARM ni sans écran : l'hôte de Crush ne peut jamais être l'hôte du
serveur MCP, seule une autre machine du réseau peut l'être.
"""
...
def _msg_injoignable() -> str:
    """Diagnostic d'un MCP muet, qui nomme la machine et non un port.

    Le message citait `settings.fusion_mcp_port`, réglage qui n'existe pas :
    la panne la plus banale — Fusion fermé — se transformait donc en
    AttributeError, et l'opérateur n'apprenait rien. L'URL, elle, existe et
    porte l'information utile : quelle machine on interroge. C'est là qu'est
    l'erreur de configuration la plus fréquente, puisque la valeur par défaut
    vise 127.0.0.1 alors que Fusion tourne forcément ailleurs.
    """
    return (
        f"Fusion 360 MCP injoignable sur {settings.fusion_mcp_url}. "
        "Fusion 360 doit être ouvert et son serveur MCP activé sur une machine "
        "Windows ou macOS ; il ne tourne pas sur l'hôte de Crush. Si Fusion "
        "tourne sur un autre poste, pointer FUSION_MCP_URL vers l'adresse de "
        "ce poste (Tailscale) au lieu de 127.0.0.1."
    )
        ...
        try:
            resp = await self._post(
                "initialize",
                {
                    ...
                    "capabilities": {},
                    "clientInfo": {"name": "crush", "version": "3.0"},
                },
            )
    ...
    async def _dispatch(
        ...
        if not _client._session_id:
            ...
            if not ok:
                ...
                ok = await _client.initialize()
                if not ok:
                    return ToolResult(content=_msg_injoignable(), is_error=True)
        ...
        try:
            ...
            # Session invalidee : la prochaine tentative refera un initialize
            # au lieu de rejouer un Mcp-Session-Id que le serveur a oublie.
