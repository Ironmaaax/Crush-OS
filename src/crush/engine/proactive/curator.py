...
from crush.kernel.contracts import SkillLifecycle
from crush.kernel.paths import MEMORY_DATA_DIR
from crush.kernel.schemas import DecayPolicy, Fact, FactStatus, SkillStatus
...
_PROTECTED_PATHS: list[str] = [
    ...
    "config/approvals.py",
    "config/backends.py",
    "src/crush/engine/vocab.py",  # vocabulaires fermés PHASE 0
    "src/crush/engine/audit.py",  # audit log gate
    "src/crush/engine/mission/governance.py",  # gate composite
    "src/crush/engine/mission/schemas.py",  # contrat de données
    "src/crush/providers/memory/schemas.py",
    "src/crush/kernel/schemas.py",  # foyer canonique (Phase A)
    "src/crush/app.py",  # ex-main.py (Phase B)
    "main.py",  # shim racine encore protégé
    "CDC_crush_evolution.md",  # cahier des charges
]
