...
from crush.engine.mission.schemas import Project, ProjectStatus, StepStatus
from crush.kernel.contracts import LLMProvider, MemoryIngest
...
class Reflexion:
    ...
    async def _analyse(self, project: Project) -> MissionLesson | None:
        ...
        try:
            raw = await self._llm.complete(
                ...
                stream=False,
                # Rattaché à la mission : sans le préfixe `mission:`, ce coût
                # échappe au BudgetGuard du projet et au tableau de bord.
                context=f"mission:{project.id}",
            )
    ...
    def _format_lesson_text(project: Project, lesson: MissionLesson) -> str:
        ...
        Le mot-clé 'crush decided X' aide l'extracteur PHASE 3 (mapping prompt
        ...
        if lesson.corrective_action:
            lines.append(f"Action corrective : crush decided {lesson.corrective_action}")
        else:
            lines.append(
                "Action corrective : crush decided continuer cette approche pour "
