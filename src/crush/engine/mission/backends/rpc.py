...
Architecture (transport fichiers — fonctionne local ET Docker via volume monté) :
  1. Génère crush_tools.py (stubs RPC) dans rpc_dir à l'intérieur du workspace
...
from crush.engine.approval_checker import get_approval_checker
from crush.engine.mission.backends.base import ExecutionBackend
from crush.kernel.contracts import ToolRegistry
...
_STUB_TEMPLATE = textwrap.dedent("""\
    \"\"\"Stub crush_tools — généré par ScriptRPCRunner. Ne pas modifier.\"\"\"
...
def _build_stub(rpc_dir: str, tools: list[str]) -> str:
    """Génère le module crush_tools.py avec une fonction par outil."""
    lines = [
...
class ScriptRPCRunner:
    ...
    async def run(
        ...
        """
        Exécute script dans le backend avec stubs crush_tools disponibles.
        ...
        rpc_dir = self._workspace / ".crush_rpc" / run_id
        ...
        (rpc_dir / "crush_tools.py").write_text(
