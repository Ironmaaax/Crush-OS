...
from crush.kernel.file_lock import exclusive_file_lock
from crush.kernel.paths import WORKSPACE_DIR as _WORKSPACE
...
class ProjectStore:
    ...
    def create_project(self, mission: str, title: str, timeout_minutes: int = 30) -> Project:
        ...
        (workspace / ".crush").mkdir(parents=True, exist_ok=True)
    ...
    def save_project(self, project: Project) -> None:
        state_file = Path(project.workspace_path) / ".crush" / "state.json"
    ...
    def load_project(self, project_id: str) -> Project | None:
        state_file = WORKSPACE_DIR / project_id / ".crush" / "state.json"
    ...
    def list_projects(self) -> list[Project]:
        ...
        for state_file in WORKSPACE_DIR.glob("*/.crush/state.json"):
    ...
    def append_log(self, project: Project, entry: LogEntry) -> None:
        log_file = Path(project.workspace_path) / ".crush" / "logs.jsonl"
    ...
    def claim_step(self, project_id: str, step_id: str, worker_id: str) -> bool:
        ...
        """
        crush_dir = WORKSPACE_DIR / project_id / ".crush"
        crush_dir.mkdir(parents=True, exist_ok=True)
        claims_file = crush_dir / "step_claims.json"
        lock_path = crush_dir / "claims.lock"
    ...
    def release_step_claim(self, project_id: str, step_id: str) -> None:
        ...
        claims_file = WORKSPACE_DIR / project_id / ".crush" / "step_claims.json"
        lock_path = WORKSPACE_DIR / project_id / ".crush" / "claims.lock"
