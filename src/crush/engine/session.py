...
from crush.kernel.contracts import SessionStore
from crush.kernel.schemas import Session

# Re-export pour compat des call-sites historiques (`from crush.engine
...
class SessionManager:
    ...
    Phase C : `store` est typé sur le Protocol `crush.kernel.contracts
    .SessionStore` (CDC §A.1.3), pas sur l'implémentation concrète
    `crush.providers.memory.sessions.SessionStore`. Le manager dépend
    ...
    Conséquences :
    - Plus aucun `from crush.providers.memory.sessions import SessionStore`
