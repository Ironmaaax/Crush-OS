...
class BudgetGuard:
    ...
    def __init__(
        ...
        # Total global du mois, tenu en mémoire — cf. `_seed_from_history`.
        self._global_memo: float = 0.0
    ...
    def _seed_from_history(self) -> None:
        """Recharge les dépenses du mois courant depuis les fichiers JSONL.

        Amorce aussi le total global en mémoire (`_global_memo`), tenu à jour
        ensuite par `noter_depense`. Le relire du disque à chaque appel LLM
        rouvrirait tous les fichiers du mois cinq à six fois par échange, pour
        une valeur que l'on connaît déjà.
        """
        try:
            today = date.today()
            ...
            while d <= today:
                ...
                d += timedelta(days=1)
            self._global_memo = total
            logger.debug(
                "BudgetGuard amorcé",
                projects=len(self._project_spent),
                global_usd=round(total, 4),
            )
    ...
    # ── Alerte sur dépense constatée ──────────────────────────────────────────
    #
    # `reserve()` est une autorisation PRÉALABLE, et n'est appelée que par le
    # worker de mission et l'exécuteur proactif. La conversation et l'indexation
    # mémoire — de loin l'essentiel de la facture — n'y passent jamais : le
    # plafond global pouvait donc être franchi sans que rien ne se déclenche.
    # D'où ce second chemin, a posteriori, branché sur chaque dépense réelle.
    #
    # Alerte seulement, pas d'arrêt : couper la parole à l'assistant au milieu
    # d'une phrase serait un remède pire que le mal, et le fournisseur applique
    # de toute façon son propre plafond.

    async def noter_depense(self, cout_usd: float) -> None:
        """Ajoute une dépense au total du mois et alerte si un seuil est franchi."""
        if not self._enabled or cout_usd <= 0:
            return
        self._global_memo += cout_usd

        limite = self._monthly_usd
        if self._bus is None or limite <= 0:
            return

        ratio = self._global_memo / limite
        # Le plafond d'abord : au-delà de 100 %, l'alerte « approche » n'a plus
        # d'intérêt et on ne veut qu'un seul message.
        for seuil, cle in ((1.0, "global:plafond"), (self._warn_ratio, "global:alerte")):
            if ratio >= seuil and cle not in self._warned:
                self._warned.add(cle)
                logger.warning(
                    "BudgetGuard: seuil global franchi",
                    depense=round(self._global_memo, 4),
                    limite=limite,
                    pct=f"{ratio * 100:.0f}%",
                )
                await self._bus.publish(
                    BudgetThresholdReached(ratio=ratio, provider="global", scope="global")
                )
                return
