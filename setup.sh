...
if [[ "${1:-}" == "--ci" ]]; then
  echo "CRUSH V3 — setup --ci (mode non-interactif)"
...
EOF
  ...
  if command -v uv > /dev/null 2>&1; then
    ...
    # cf. pyproject.toml. Le socle doit rester installable sur un Pi.
...
if [[ -z "$PYTHON_BIN" ]]; then
  ...
  echo ""
  echo "CRUSH — aucun runtime pre-packaged detecte."
  echo "  A) Telecharge une release Crush avec bundle offline"
...
if ! "$PYTHON_BIN" -c "import crush.setup_app" 2>/dev/null; then
  UV_CMD="uv"
...
echo ""
exec "$PYTHON_BIN" -m crush.setup_app
