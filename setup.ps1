...
Invoke-CrushOneDriveGuard -ProjectRoot $PSScriptRoot -NonInteractive:$Ci -RelaunchCommand "setup"
...
function Ensure-CrushPackage {
    param([string]$PythonPath)
    & $PythonPath -c "import crush.setup_app" 2>$null
    if ($LASTEXITCODE -eq 0) { return }
    $uvCmd = Get-BundledUv
    if (-not $uvCmd) { throw "crush package missing in venv and uv unavailable." }
...
if ($Ci) {
    Write-Host "CRUSH V3 - setup --Ci (mode non-interactif)" -ForegroundColor Cyan
    $py = Get-CrushPython
    if (-not $py) {
        Ensure-Uv
    ...
    & $py -c "from crush.kernel.setup_layout import ensure_runtime_layout; ensure_runtime_layout()"
...
Write-Host ""
Write-Host "CRUSH - Configuration web locale" -ForegroundColor Cyan
Write-Host ""
...
if (-not $python) {
    ...
    Write-Host ""
    Write-Host "Option A (recommandee) : telecharge une release Crush avec bundle offline" -ForegroundColor White
...
Ensure-CrushPackage -PythonPath $python
